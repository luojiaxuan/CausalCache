...
\definecolor{acc}{RGB}{0,140,130}
\definecolor{ink}{RGB}{70,70,70}
\definecolor{chipgray}{RGB}{225,225,225}
\definecolor{chipdark}{RGB}{120,120,120}
\newcommand{\frozenmark}{\textcolor{blue!45!cyan}{\ding{100}}}
\begin{document}
\begin{tikzpicture}[
  font=\small, ink/.style={draw=ink, thick},
  tok/.style={draw=ink, minimum size=4.2mm, inner sep=0pt},
  gtok/.style={tok, fill=chipgray},
  htok/.style={tok, draw=acc, very thick, fill=acc!12},
  ...
  tarr/.style={-{Stealth[length=2.2mm]}, acc, very thick},
...
\begin{scope}[shift={(0,-1.05)}]
  ...
  \foreach \i in {4,...,6}  \node[gate, draw=acc, text=acc] (g\i) at (0.46*\i, -0.52) {\scriptsize 1};
  \foreach \i in {7,...,11} \node[gate] (g\i) at (0.46*\i, -0.52) {\scriptsize 0};
  \node[lbl, anchor=east] at (-0.35, -0.52) {gate $g$};
  % group braces / labels
  \node[lbl, anchor=north, align=center, font=\scriptsize] at (0.69, -0.95) {task +\\summaries};
  \node[lbl, anchor=north, align=center, font=\scriptsize, text=acc] at (2.30, -0.95) {restored\\history img};
  ...
  \node[draw=acc, very thick, rounded corners=2pt, fill=acc!8, font=\small,
  ...
  \node[lbl, text=acc, font=\scriptsize, anchor=west] at ($(dkv.east)+(0.05,0.22)$) {$\times\, g$};
  % bypass note
...
\begin{scope}[shift={(8.6,2.35)}]
  ...
  \node[ink, rounded corners=2pt, right=4.5mm of pa, align=center, inner sep=3pt]
    (pol1) {policy+\textsc{hgkv} \frozenmark};
  ...
  \node[ink, rounded corners=2pt, inner sep=3pt, below=4.5mm of dot] (ss) {%
    \begin{tikzpicture}[baseline]
      ...
      \foreach \i/\h in {1/0.34, 6/0.30, 8/0.26, 9/0.32}
        \fill[acc] (0.32*\i,-0.13) rectangle (0.32*\i+0.20,-0.13+\h);
    \end{tikzpicture}};
  ...
  \node[ink, rounded corners=2pt, right=4.5mm of pb, align=center, inner sep=3pt]
    (pol2) {policy+\textsc{hgkv} \frozenmark};
