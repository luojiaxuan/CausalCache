...
\begin{axis}[
  ...
  xmin=-0.4, xmax=8.6, ymin=24.5, ymax=36.5,
...
\node[font=\scriptsize, text=slate, anchor=north west, align=left]
  at (axis cs:-0.15,26.1) {$B{=}0$: identical};
% pooled 注记
\node[font=\small, text=ink, anchor=north west, align=left]
  at (axis cs:0.35,36.3)
  {pooled over $B$: $+4.1$pp\; ($p{=}0.004$)};
\end{axis}
\end{tikzpicture}
\end{document}
