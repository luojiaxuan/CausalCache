...
and task horizon. \pending{Primary-policy selection is currently blocked:
frozen local gates rejected both GUI-Owl variants.  Insert no method-effect
claim until a new validated reference is preregistered and passes its gate.}
\end{abstract}
...
\end{quote}
The archive stores the original screenshots, raw action arguments, canonical
action, UI delta, and metadata. Restoring an event exposes the raw arguments;
the coarse target remains exclusive to its low-fidelity record.
...
\pending{The primary frozen policy remains unresolved.  GUI-Owl-1.5-8B-Instruct
and GUI-Owl-1.5-8B-Think passed native interface checks but were rejected by
fixed-denominator AndroidWorld success upper bounds of 30/62 and 29/62,
respectively.  Restoration-label generation is paused.}
