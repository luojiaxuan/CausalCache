...
query-conditioned multi-budget gate. The v2 reference is screened for exact
...
reference stack remains rejected. The preregistered v2 estimand fixes
GUI-Owl-1.5-8B-Instruct as a stable self-behavior reference; insert no
method-effect claim until the substrate and untouched restoration gates pass.}
\end{abstract}
...
\end{equation}
Its deterministically decoded output is normalized to one exact native
MobileUse tool call and used as the canonical action $a_t^*$. The substrate
...
than being removed. For teacher forcing, the processor's native assistant prefix
is followed by the fixed nonsemantic carrier ``Action: Execute the selected
mobile action.'' and then a deterministic MobileUse tool call. This preserves
the native response envelope without leaking the reference's generated target
description into every coalition.
...
\texttt{<tool\_call>} through the closing tag; the fixed Action carrier is
context but is not scored. Action-type match, canonical coordinate-bin match, normalized
...
The v2 primary policy is frozen to GUI-Owl-1.5-8B-Instruct
\cite{xu2026mobileagent35} at model revision
\texttt{06d5faec}, using bfloat16 and
deterministic decoding. The earlier GUI-Owl and UI-TARS expert-alignment gates
remain valid negative v1 results; v2 changes the estimand rather than lowering
those thresholds.
