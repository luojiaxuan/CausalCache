...
**HGKV 通过全部预注册 gate,选点 s150,且选择性在真·前一帧对照下复核成立:**
- v1 口径(R 臂 = 部署 recent 选择器,k=1 时恢复帧与当前截图逐字节相同):
  `did_select = +0.0111,95% CI [+0.0074, +0.0150]`(ci_low > 0 ✅),
  `A_S = +0.0139 > 0`,`|A_r| = 0.0087 < 0.02`,`wrong_drift_abs = 0.0085 < 0.02`;
- **真·前一帧复核探针**(R′ 臂恢复事件 s−2 的 post 帧,即前一观测;94/94 组
  前一帧≠当前帧;`true_recent_probe_dev.json`):
  `did_select_true = +0.0095,95% CI [+0.0056, +0.0138]`,60/94 组占优,
  `|A_r_true| = 0.0044` 仍在 cap 内 —— **HGKV 的选择性不依赖弱对照**。
桌面数据 + DiD 目标下,门控 KV adapter 学到了**内容选择**而不是"见历史图就放大"
—— 旧 GUI-Odyssey 线的两个失败模式(四臂遗漏 RA / legacy SA−R0 假阳性)均未复现。

**重要修正(2026-07-27,由该探针得出)**:冻结基线的
`frozen_selection_effect = +0.0333` 是相对**冗余副本**的;对真·前一帧,
`true_selection_effect = +0.0013,CI [−0.0242, +0.0253]`,**不显著**——
冻结模型并不天然偏好目标相关旧帧,选择性是 HGKV 训练出来的。这一修正
强化 adapter 的必要性,但取代此前"冻结模型已显著受益于目标相关旧帧"的表述。
v2 语料冻结定义:"next recent" = 最近的**不同帧**;部署重复帧现象单独披露。
...
- ungated 对照说明 last_8 k/v 的层位/参数量本身就能学到部分选择性;
  门控在此之上仍有显著净增益。
- 冻结基线(修正后):相对冗余副本 `+0.0333 [+0.0118, +0.0567]`;相对
  真·前一帧 `+0.0013 [−0.0242, +0.0253]`(**不显著**)。目标相关旧图与有信息的
  前一帧对冻结模型价值相当——选择性优势由 HGKV 产生,不是基线自带。
