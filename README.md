...
- formal label rollout 已在 reference parse-failure repair 后扩到 Hyper00/Hyper01 共 12xH200：24 个 resumable workers、每卡 2 个 processes。2026-07-20 03:28 UTC 已有 5,317/11,746 terminal states（45.3%）；最近速率约 26.8 states/min，剩余 ETA 4--5.5 小时。运行入口见 [`data/results/set_utility_variable_history_labels_v1/`](data/results/set_utility_variable_history_labels_v1/README.md)。
...
| Variable-history formal labels | [HF dataset](https://huggingface.co/datasets/gavinlaw/causalcache-set-utility-variable-history-mobile) | `RUNNING`；5,317/11,746 terminal states at 2026-07-20 03:28 UTC；完成后发布 immutable revision |
...
| Variable-history formal label run | [`data/results/set_utility_variable_history_labels_v1/`](data/results/set_utility_variable_history_labels_v1/README.md) | `RUNNING`；12xH200 / 24 workers；state/microbatch atomic resume；repair=`415f608` |
