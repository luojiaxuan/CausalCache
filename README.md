...
- held-out selector v1 已在读取 evaluation restoration labels 前冻结：805-state label-blind feature/cache 和两类
  ...
  at-most-`B` selections 已密封进 Git。下一步只按该 artifact 生成 sparse/exact truth；只有真实 utility GO 且
  policy replay latency 过线才进入 closed-loop。[合同](docs/set_utility_heldout_v1.md)与[artifact](data/results/set_utility_heldout_v1/README.md)。
