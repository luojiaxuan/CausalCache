...
> 68 个为坐标越界。因此先前 B0=`17/117`、B4=`27/117` 及其显著性不得引用。
> `maximum_history_images=0` 只证明 B0 没有图片泄漏，不能证明 prompt 正确。
> 替代实验改用与 AndroidWorld 相同的 official-faithful 多轮 renderer、模型原始完整
> response、pre-action screenshot 和 UNKNOWN-step parser，配置为
...
> frozen GUI-Owl strict-117：B0/B1/B2/B3/B4 分别为
> `33/31/31/34/35`，即 `28.21/26.50/26.50/29.06/29.91%`。相对 B0：
> B1=`−1.71 pp [−7.69,+4.27]`、B2=`−1.71 pp [−7.69,+4.27]`、
> B3=`+0.85 pp [−5.13,+6.84]`、B4=`+1.71 pp [−5.98,+9.40]`；
> paired CI 全部跨 0。**Recent-B 不呈单调剂量效应，也没有证据表明用满更多
> recent history images 会稳定改善成功率。** B1/B2/B3 policy failures 均为 0，
> maximum history=1/2/3；16 个模拟器、两路 B3 policy 和 nested dockerd
> 已清理，canonical compute container 保留。最终 summary、task matrix 与 provenance 见
...
| arXiv 作者预览与源码包 | [`paper/main_arxiv.tex`](paper/main_arxiv.tex)；[`paper/package_arxiv.sh`](paper/package_arxiv.sh)；[arXiv:2608.22577](https://arxiv.org/abs/2608.22577) | 与 `main.tex` 共用完整正文；`make arxiv arxiv-source` 可复现；v1 为 `submit/7983880`；2026-08-24 PDT 已正式提交 `submit/7989797` replacement（`cs.AI`、无 cross-list、arXiv non-exclusive license），服务器 TeX Live 2025 编译成功，状态 `processing`；replacement 正文锚定历史 commit `1dfd5b0a6aa0cb7c2ca8576c7d42fd862a6baf88`，作者为 4 人并已写入 JHU/清华单位与机构邮箱 |
...
| MobileWorld-Memory / OSWorld 2.0 substrate | [`docs/mobileworld_memory_osworld2_v1.md`](docs/mobileworld_memory_osworld2_v1.md)；[`official-faithful MobileWorld B0/B4 v2`](data/results/mobileworld_frozen_gui_owl_official_b0_b4_v2/README.md)；[`official-faithful MobileWorld B0–B4 curve v2`](data/results/mobileworld_frozen_gui_owl_official_b1_b2_b3_v2/README.md)；[`invalid MobileWorld B4 v1`](data/results/mobileworld_frozen_gui_owl_benchmark_v1/README.md)；[`invalid MobileWorld B0/B4 v1`](data/results/mobileworld_frozen_gui_owl_b0_b4_v1/README.md)；[`OSWorld 2.0 result`](data/results/osworld_v2_frozen_gui_owl_benchmark_v1/README.md)；[`mobile manifest`](data/manifests/mobileworld_memory_split_v1.json)；[`OSWorld 2.0 manifest`](data/manifests/osworld_v2_memory_split_v1.json)；[`initial 5-GPU plan`](data/manifests/osworld_v2_gpu_shards_v1.json) | MobileWorld v2：B0–B4=`28.21/26.50/26.50/29.06/29.91%`，所有相对 B0 paired CI 跨 0；v1=`INVALID_PROTOCOL_MISMATCH`；OSWorld 2.0 full-108 corrected wall=46:31.472、strict mean=0.006692 |
| **CausalCache-RL 线**(2026-08-04 pivot;agentic RL 联训 selector+HGKV) | 代码与文档 [`rl/`](rl/README.md);方法 [`rl/docs/rl_method.md`](rl/docs/rl_method.md);预注册 [`rl/docs/rl_pivot_contract.md`](rl/docs/rl_pivot_contract.md);**交接包** [`rl/docs/rl_handoff_v1.md`](rl/docs/rl_handoff_v1.md) | v1 campaign 完成:20 迭代 + 双轮 held-out = RL 81/240 vs recent-B 88/240(Δ=−2.9pp,p=0.345,CI 跨零)。**判为配置失败非假设失败**(两条通道均未训动:selector 位移 4.4%、`lora_b` 范数 0.04);v2(lr 修复)运行中 |
| RL 任务切分与可学带 | [`rl/data/manifests/osworld_rl_train_v1.json`](rl/data/manifests/osworld_rl_train_v1.json)(241)、[`heldout_v1`](rl/data/manifests/osworld_rl_heldout_v1.json)(120)、[`band_v2`](rl/data/manifests/osworld_rl_train_band_v2.json)(44) | Git canonical;held-out 120 从未参与任何开发环节 |
| RL v1 轨迹 / 评测原始结果 | hyper00 `/data04/jaxan/rl/iter-{1..20}/`(~10GB)、`/data04/jaxan/rl/eval-iter{12,20,20-r2}/`(~3GB);汇总数已入 [`rl/README.md`](rl/README.md) | intended `gavinlaw/causalcache-rl-osworld-trajectories`;`PENDING_HF_UPLOAD` |
| AgentNet ubuntu 帧集(枚举/步级训练用) | [HF dataset `gavinlaw/causalcache-agentnet-frames-ubuntu-v3`](https://huggingface.co/datasets/gavinlaw/causalcache-agentnet-frames-ubuntu-v3)(私有);6 个 tar 分片约 25GB(29,398 张)+ screening manifest(6,003 态) | **已上传**,revision `31fedb85`;用途:hyper↔tilde 中转(两端 ssh 双向认证失败,按全局规则走 HF,不过笔记本);本地副本 h00/h01 `/data04/jaxan/agentnet-frames-v3`、tilde `~/causalcache/agentnet` |
| RL 环境契约 | 容器 `jaxanluo/sglang-omni:dev`;**transformers 必须 5.6.0**(冻结守卫 fail-closed);VM `happysixd/osworld-docker:latest` + OSWorld @ `0514b9a2` | 见交接包第 4 节;**rollout 需 `/dev/kvm` + docker,Slurm 受管节点不可用**(tilde 实测) |
