...
- 100% contextual 双模型训练已完成：DeepSets/Set Transformer 都在 epoch 1 最佳，tune objective 分别为
  `0.30927/0.32233`。新的 1,063-state conditional-greedy schedule 含 10,368 个 coalitions；真实 `D(S)`
  正在 Hyper00/Hyper01 各 4×H200 生成，仍未访问 evaluation。
...
| Contextual full predictor training v4 | [`summary`](data/results/set_utility_contextual_training_full_v4/README.md)；Hyper00 persistent root | DeepSets `0.30927`；Set Transformer `0.32233`；true selector utility pending；`PENDING_HF_UPLOAD` |
