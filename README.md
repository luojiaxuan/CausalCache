...
- formal label rollout 正在 Hyper00/Hyper01 的 8xH200 上运行：24 个 resumable workers、每卡 3 个 in-flight process。初始 86 states / 3,232 labels 已验证，覆盖 `n_t=5..9`，全部完成且 KL 合法；运行入口见 [`data/results/set_utility_variable_history_labels_v1/`](data/results/set_utility_variable_history_labels_v1/README.md)。
...
| Variable-history formal labels | [planned HF dataset](https://huggingface.co/datasets/gavinlaw/causalcache-set-utility-variable-history-mobile) | `PENDING_CREATION_AND_UPLOAD`；variable-history source/tokens 已就绪，尚未生成 restoration labels |
...
| Variable-history formal label run | [`data/results/set_utility_variable_history_labels_v1/`](data/results/set_utility_variable_history_labels_v1/README.md) | `RUNNING`；8xH200 / 24 workers；state/microbatch atomic resume |
