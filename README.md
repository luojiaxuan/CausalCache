...
> Non-mainline v4 Frozen-base residual：v3 的 learned additive base 在 Fresh16 只有 `0.590359`，弱于历史
> independent gate 的 `0.712379`，因此 v3 不能排除 joint training 先损坏 base 的解释。隔离分支
> `luojiaxuan/set-conditioned-v4-frozen-base-residual` 冻结历史 5-seed independent ensemble 的全部
> `200→88→88→1` 参数，只训练 chronological pair correction
> `[z_i,z_j,z_i*z_j,|z_i-z_j|,q64]→64→1`。末层零初始化把 epoch 0 精确定义为原 base；correction
> target 与 base 同为 normalized score unit，不沿用 v3 raw residual。Formal58 的 model selection 重放 25 个
> fold-clean base；真实缓存检查已在 `1e-12` 内复现五个历史 OOF ratio 和 mean=`0.9063764691683989`。
> Fresh16 仍只是 consumed development，primary 是继承 v3 4/5 双门的 safe residual 相对 frozen base；历史
> independent artifact 只用于逐 state/seed exact replay audit，不另定义 primary baseline。正式 A=`ea3d53b…`、
> B=`8e8bfd9…` 的 CPU-only run 得到 `NO_DEVELOPMENT_EVIDENCE_FOR_FROZEN_BASE_RESIDUAL`：base checkpoint
> bytes 训练前后完全一致；safe residual 在 Fresh16 的 48/48 state 都等于 frozen base，normalized/raw/n=4
> delta 全为 0；unguarded 只改 1 state 且 mean normalized delta=`-9.08e-7`。五项 route 全失败，confirm20 保持
> 0，同一 Formal58/Fresh16 不再调 v5。两仓 private HF immutable revisions 为 model=`1641b90a…cdc3`、
> development=`d38513e4…e73f`，均完成 force-download hash replay。完整结果见
> [`data/results/set_conditioned_v4_frozen_base_residual_dev_v1/`](data/results/set_conditioned_v4_frozen_base_residual_dev_v1/)。
...
> set-conditioning 两个 GO gate 都未通过；matched-NLL、closed-loop 与 confirm 仍未执行。formal-58 train-only cache 的 v1 Source-A
...
> namespace 上完成；旧 dev-5、confirm、matched-NLL 与 closed-loop 仍未打开。fresh-16 evaluation Source-A 冻结了 exact 16 trajectories / 48 states / 144
...
都已闭合；旧 dev-5、confirm、matched-NLL 与 closed-loop 仍保持 locked。
...
- [x] 冻结并 push non-mainline v4 Frozen-base residual Source-A；
- [x] 从 clean pushed Source-A 机械生成并单独 push 唯一 Execution-B；
- [x] 完成 v4 Formal58 residual-only train-seal、Fresh16 consumed-development evaluation、两仓 private-HF
  publication/fresh-download replay 与 Git compact result；confirm20、matched-NLL、closed-loop 保持 0；
- [ ] 整理论文与复现实验配置。
...
- Non-mainline v4 Frozen-base residual protocol: [`docs/set_conditioned_v4_frozen_base_residual.md`](docs/set_conditioned_v4_frozen_base_residual.md)
- Non-mainline v4 Source-A config: [`code/configs/causalcache_set_conditioned_v4_frozen_base_residual_development_v1.json`](code/configs/causalcache_set_conditioned_v4_frozen_base_residual_development_v1.json)
- Non-mainline v4 Execution-B freeze: [`code/configs/causalcache_set_conditioned_v4_frozen_base_residual_runner_v1.json`](code/configs/causalcache_set_conditioned_v4_frozen_base_residual_runner_v1.json)
- Non-mainline v4 source/Execution-B validator: [`code/scripts/validate_set_conditioned_v4_contract.py`](code/scripts/validate_set_conditioned_v4_contract.py)
- Non-mainline v4 scientific runner: [`code/scripts/run_set_conditioned_v4_exploration.py`](code/scripts/run_set_conditioned_v4_exploration.py)
- Non-mainline v4 result: [`data/results/set_conditioned_v4_frozen_base_residual_dev_v1/`](data/results/set_conditioned_v4_frozen_base_residual_dev_v1/)；route `NO_DEVELOPMENT_EVIDENCE_FOR_FROZEN_BASE_RESIDUAL`
- Private v4 model artifacts: <https://huggingface.co/gavinlaw/causalcache-set-conditioned-v4-frozen-base-residual-exploration-mobile/tree/1641b90a4ebb05037e4710738a78e2c79f81cdc3>
- Private v4 development artifacts: <https://huggingface.co/datasets/gavinlaw/causalcache-set-conditioned-v4-frozen-base-residual-development-mobile/tree/d38513e4c3a22e02dcef1277472c5adf8667e73f>
...
| Set-conditioned v4 Frozen-base residual | [result](data/results/set_conditioned_v4_frozen_base_residual_dev_v1/) / [private HF model](https://huggingface.co/gavinlaw/causalcache-set-conditioned-v4-frozen-base-residual-exploration-mobile/tree/1641b90a4ebb05037e4710738a78e2c79f81cdc3) / [private HF dataset](https://huggingface.co/datasets/gavinlaw/causalcache-set-conditioned-v4-frozen-base-residual-development-mobile/tree/d38513e4c3a22e02dcef1277472c5adf8667e73f) | A=`ea3d53b` / B=`8e8bfd9`；tag `set-conditioned-v4-frozen-base-residual-development-v1`；model=`1641b90a…cdc3` / development=`d38513e4…e73f` | `NO_DEVELOPMENT_EVIDENCE_FOR_FROZEN_BASE_RESIDUAL`；safe 48/48 等于 frozen base；两仓 fresh-download verified；confirm20/matched-NLL/closed-loop locked |
