...
- formal label rollout 已在 reference parse-failure repair 后扩到 Hyper00/Hyper01 共 12xH200，并通过 deterministic state lanes 从 24 扩到 36 workers。2026-07-20 03:54 UTC 已有 5,983/11,746 terminal states（50.9%）；36-worker 首分钟约 27 states/min，尚未显示显著加速，当前剩余 ETA 3.5--5 小时。运行入口见 [`data/results/set_utility_variable_history_labels_v1/`](data/results/set_utility_variable_history_labels_v1/README.md)。
...
| Variable-history formal labels | [HF dataset](https://huggingface.co/datasets/gavinlaw/causalcache-set-utility-variable-history-mobile) | `RUNNING`；5,983/11,746 terminal states at 2026-07-20 03:54 UTC；完成后发布 immutable revision |
