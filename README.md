...
- held-out selector v1 已在读取 evaluation restoration labels 前冻结：同时密封两个 100% checkpoints，在 805 个
  trajectory-disjoint states 上用 conditional-greedy at-most-`B` 与 recent/OCR-RGB 比较；只有真实 utility GO 且
  policy replay latency 过线才进入 closed-loop。label-blind feature、一次编码多 subset 打分、selection merge 与
  sparse schedule 接口已实现；evaluation restoration labels 尚未打开。[合同](docs/set_utility_heldout_v1.md)。
