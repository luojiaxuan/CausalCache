...
- held-out selector v1 已在读取 evaluation restoration labels 前冻结：805-state label-blind feature/cache 已在
  Hyper00/Hyper01 完成并严格合并（exact 320、large 720、overlap 235、label reads 0），两个 100% checkpoints
  正在并行生成 conditional-greedy at-most-`B` selections。selection merge、sparse truth schedule 与正式 reducer
  已实现；只有真实 utility GO 且 policy replay latency 过线才进入 closed-loop。[合同](docs/set_utility_heldout_v1.md)。
