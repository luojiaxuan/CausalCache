...
- 100% contextual 双模型训练已完成：DeepSets/Set Transformer 都在 epoch 1 最佳，tune objective 分别为
  ...
  evaluation，下一步做 train-side on-policy enrichment。
...
| Contextual full tune truth v4 | [`summary`](data/results/set_utility_contextual_tune_on_policy_full_v4/README.md)；Hyper00 persistent root | 1,063/1,063；Set Transformer `0.44985` vs recent `0.45192`；`FULL_DATA_NO_GO`；`PENDING_HF_UPLOAD` |
