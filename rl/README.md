...
这些是**噪声梯度源**——可学带 0<p<1 的准入条件天然偏爱"评测器/环境本身
抖动"的任务,GRPO 在它们身上学到的是评测器怪癖而非行为改进。

- 早先列为观察项的 **4c26e3f3 已判定为噪声**(0.17/0.67/0.17/0.33/0.33/0.50),
  当时的 i5→i8 跳变是运气不是学习——事前的谨慎是对的;
- **不在本 campaign 中途裁带**:换带会让训练带成功率跨迭代不可比,
  代价大于收益。记为 band v3 候选,下一轮 campaign 生效。
