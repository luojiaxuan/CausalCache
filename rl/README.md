...
20 迭代看不出上行。**KL 全程在 0.18-0.28,但**这不是「信任域生效」而是
「策略几乎没被改动」**——mean_kl 是每 episode 全部动作 token 的 KL 之和,
折合每 token 约 1e-3 nats。详见下文「追加更正」。
...
- held-out 是 **τ=0 部署口径 + 固定 120 任务全配对**,方差小得多,是回答
  "RL 有没有用"的正确工具;
- 两者不矛盾但也**都不构成结论**:方向读单轮双侧 p=0.238、95% CI 跨零,
  训练带持平。正式两轮评测(iter-20)才是本阶段的裁决;
...
## v2 campaign(2026-08-08 起):**上线门槛 iter-1 即通过**

| | v1(20 迭代累计) | **v2(仅 iter-1)** | 每迭代速率提升 |
|---|---|---|---|
| `selector_drift` | 0.044 | **0.1038** | ≈47× |
| adapter `lora_b` 范数 | 0.0399 | **0.0790** | ≈40× |

门槛为 `selector_drift` ≥0.02/迭代,实测超出 5 倍;`lora_b` **一个迭代就到
v1 二十个迭代的两倍**。两条可学通道均已实质更新——v1 的"KL 平台"确系
"策略未被改动",现已排除。**注意:参数动 ≠ 成绩好**,本关只证明实验恢复了
检验假设的能力,结论仍待 iter-20 双轮评测。

iter-1 因下述端口锁事故只收到 33/48 条(4 组可用、4 次优化步),iter-2 起
数据补齐后每迭代 7-11 步。

**iter-2(48/48,7/8 组可用,11 优化步)累计位移 vs 随机初始头:**

| 指标 | v1 @iter-20 | **v2 @iter-2** |
|---|---|---|
| 累计 ‖Δw‖/‖w₀‖ | 0.044 | **0.2474** |
| 与初始头 top-2 选择重合率 | 0.867 | **0.141** |
| 打分 Spearman | 0.956 | **−0.252**(排序反转) |

**参数被大幅改写,门槛问题彻底解决。**

### 观察:selector 正倒向 recent(两种读法,尚未分辨)

**选中帧的归一化年龄:随机初始 0.733 → iter-2 0.329**(1=最老,0=最新)。
随机头偏好老帧,而 GRPO 学到的第一件事是**反过来偏好近期帧**,即朝 recent-B
基线靠拢。两种解释目前都成立:

1. **这就是最优解**:冻结策略本就在 recent 窗口 prompt 上调过,近期帧确实
   让它预测更准。若如此,闭环终将 ≈ recent-B——**不是没学到,而是学到的
   最优就是 recent-B**,这本身是有信息量的负结果;
2. **是塌缩**:熵正则(λ=0.01)本为防此而设,但离线扫描已证明它对位移
   几乎无影响(0.049 vs 0.046),**这道防线实际虚设**。

这两种读法可由 oracle 枚举闭环实验分辨(全枚举最优 B=2 组合跑真实闭环:
赢不了 recent-2 → 解释 1;显著赢 → 解释 2)。该实验因此同时回答
"选择有无闭环价值"与"v2 曲线该怎么解释",是当前性价比最高的一步。

### 事故:GEN_PARALLEL=2 撞上 OSWorld 全局端口锁

为提速引入的 16 路并发使 48 条丢 15 条,失败计数与
`Timeout: The file lock '/tmp/docker_port_allocation'` 精确对应。
根因:`provider.py` 的全局文件锁罩住**端口分配 + 整个容器启动**
(约 5s/台,全局串行),`LOCK_TIMEOUT` 仅 10s,排队深度一超即整条 episode 失败。
**server 侧完好因而不触发修复遍**,日志上只表现为"收得少",极易误判为
"任务变难"。已在 h00 打补丁 `LOCK_TIMEOUT=600`(worker 每代新进程,
立即生效、无需重启循环)。**这是横向扩展的第一道硬墙**,详见
[`docs/rl_handoff_v1.md`](docs/rl_handoff_v1.md) 第 0 节。

## iter-20 正式评测(2026-08-08,双轮):**零效应,方向轻微为负**

**RL@iter-20 81/240(33.8%) vs 冻结+recent-B 88/240(36.7%),Δ = −2.9pp
(≈ −3.5 个任务);任务级配对 11 胜 17 负 92 平,双侧符号检验 p=0.345,
bootstrap 95% CI [−8.8pp, +3.3pp] 跨零。20 迭代 RL 未能超过冻结基线。**

| 轮 | RL | recent-B | Δ | 胜/负 | 符号 p |
|---|---|---|---|---|---|
| r1 | 42/120 | 43/120 | −1 | 9/10 | 1.000 |
| r2 | 39/120 | 45/120 | −6 | 6/12 | 0.238 |
| 合并 | 33.8% | 36.7% | **−2.9pp** | 11/17 | **0.345** |

协议:held-out 120(从未参与开发)× 双臂 × 2 轮 × τ=0 × 50 步 × B=2,
两轮并行同机同日;**四个臂全部收满 120 条**(完备性闸门通过,无缺口偏差)。

### 测量噪声标定(本次最重要的副产品)

| 对照 | 结果 |
|---|---|
| 同一冻结基线跨评测重测(iter-12 评测 vs iter-20 评测,策略一个比特未变) | 39/120 vs 43/120,**逐任务翻转 14/120 = 11.7%** |
| 本次双轮内 recent 臂跨轮翻转 | 8/120 = 6.7% |
| 本次双轮内 rl 臂跨轮翻转 | 13/120 = 10.8% |

**结论:整条测量管线的任务级噪声约 7-12%,总数噪声约 ±4 个任务。**由此:

- iter-12 那次 "+6" 仅略高于同系统重测的噪声,其 CI 跨零并非偶然——
  **它当时就不该被当作正向证据,现在被双轮否定并不矛盾**;
- 单轮 120 任务的检出力:观察到约 19-28 个不一致任务时,需净胜 ≈11 个任务
  (≈9pp)才达 p<0.05。**本方法若有效应,量级也小于本设计的检出下限**;
- 要压到 ±2pp 的分辨率,需要约 4-6 轮 × 120 任务(当前 ~3h/轮/臂,双轮并行)
  或扩大 held-out 规模。

### 根因诊断:**不是"记忆选择无用",是 selector 根本没被训动**

零效应有两种解释,必须分开。两个诊断把它们分开了:

**(1) 学出的 selector 与 recent-B 截然不同**(serve 的 `k_off_recent`,9555 步):

| | 实测 | 随机选帧期望(按真实 pool_size 逐步算) |
|---|---|---|
| 两帧都是最近 2 帧 | 0.5% | 2.5% |
| 恰好一帧 | 10.1% | 23.3% |
| 两帧都非近期 | **89.3%** | 74.2% |

它比随机更强烈地回避近期帧——**没有塌缩到基线**,却打平基线。

**(2) 但这个"偏好老帧"是随机初始化就有的,不是学出来的**
(`rl_selector_drift.py`,iter-0 头 vs iter-20 头,同一批审计 hidden 向量):

| 指标 | 值 | 含义 |
|---|---|---|
| 权重相对位移 ‖Δw‖/‖w₀‖ | **0.044** | 20 迭代只挪了 4.4% |
| top-2 选择重合率 | **0.867** | 与随机初始化头选出同样的帧 |
| 打分 Spearman | 0.956 | 排序几乎没变 |
| 选中帧归一化年龄 | init 0.838 → iter-20 0.875 | 老帧偏好来自**架构+初始化的几何偏置** |

**症结是有效步长**:20 迭代 × 约 30 条可用 episode ÷ grad_accum 8
≈ **总共 67 次 optimizer.step()**,selector lr 仅 1e-4。这不是一次训练,
是一段热身。

**离线反事实扫描证实了这一点**(`rl_selector_lr_sweep.py`,同一批 936 episode /
44733 轮选帧原样重放通道 A,只换 lr 与 λ_H):

| lr | λ_H | 步数 | ‖Δw‖/‖w₀‖ | 与初始头选择重合率 |
|---|---|---|---|---|
| 1e-4(实际用的) | 0.01 | 67 | **0.049** | 0.530 |
| 1e-4 | 0 | 67 | 0.046 | 0.750 |
| **1e-3** | **0.01** | 67 | **0.589** | **0.124** |
| 1e-3 | 0 | 67 | 0.587 | 0.125 |
| 1e-2 | 0.01 | 67 | 5.688 | 0.214 |
| 3e-2 | 0.01 | 67 | 17.211 | 0.224 |

- **重放保真**:lr=1e-4 复现出位移 0.049,与真实 campaign 实测的 0.044 吻合,
  说明这个离线重放可信;
- **优化器本身没坏,只是步子小了约 10 倍**:lr=1e-3 在**完全相同的数据量**下
  就能把头挪到 0.59、选择重合率降到 0.12(即基本换了一套选帧策略);
- lr≥1e-2 时权重涨到初始范数的 5.7-17 倍,是被单一累积方向支配的失控区,
  且重合率反而回升(排序退化),不可取;
- **熵系数几乎不影响位移**(0.049 vs 0.046)——**收回**此前"熵正则把头钉在
  初始点"的归因,它只轻微改变移动方向(重合率 0.53 vs 0.75),不改变幅度。

**结论**:iter-20 的零效应,主因是 selector lr 小了约一个数量级 + 优化步数
只有 67 次。**当前 campaign 检验的其实是"几乎不更新参数的 RL"**,它无法
证伪也无法支持 CausalCache 的核心假设。注意反事实的边界:它只证明参数
**会动**,不预测闭环成绩会变好(换 lr 后策略走向不同状态、采到不同数据)。

### 追加更正:**LoRA 通道同样没训动,"KL 平台"是我的误读**

复盘时顺手查了 adapter,结果推翻此前多次汇报的"KL 锁在 0.18-0.28 平台 =
信任域按设计工作":

| 量 | iter-20 实测 |
|---|---|
| `lora_a`(随机初始化,524288 参数) | 范数 6.535 |
| `lora_b`(**零初始化,唯一真正学到的部分**,131072 参数) | 范数 **0.0399** |
| 有效增量 ΔW=(α/r)·B·A 相对基座权重 | 约 **0.1% 量级** |
| iter-10 / 15 / 20 的 adapter 总范数 | 6.5336 / 6.5340 / 6.5352(几乎不变) |

且 `mean_kl` 的口径是**每条 episode 全部动作 token 的 KL 之和**
(`action_scoring.py:188` 逐 token 求和后再逐步累加),不是每 token 均值。
按每 episode 数百个动作 token 估算,**每 token KL 约 1e-3 nats 量级**——
动作分布几乎没变。

**所以"KL 平台"从来不是信任域在起作用的证据,而是"策略基本没被改动"
的证据。**我在多次汇报里把它当作正面信号,这是错的。

修正后的完整定性:**两条可学通道(selector 头 lr 1e-4、HGKV LoRA lr 1e-5)
在 67 次优化步下都没有实质更新**,整个 campaign 实际测的是
"冻结策略 + 随机初始化 selector",重复测了 20 次。下一版 campaign 必须
同时提高两条通道的有效步长,并以 `selector_drift` / adapter `lora_b` 范数
作为前 2-3 迭代的上线门槛。

**因此 iter-20 的零效应不能读作"历史帧选择对该策略无用"**,只能读作
"**本配置在 20 迭代内没有把 selector 训动**"。真正被检验并证伪的是
当前的训练配置,不是 CausalCache 的核心假设。

(可确认的弱结论:随机初始化头选出的老帧 vs recent-B,闭环成绩无可测差异——
但受限于 ±9pp 的检出下限,这只能说"差异不大",不能说"没有差异"。)

### 与训练带持平的一致性

训练带 20 迭代持平(前十 31.9% / 后十 32.4%),held-out 零效应——
**两条独立证据方向一致:20 迭代的 RL 没有产生可测量的闭环收益。**
这与"选帧—成败耦合诊断"的负结果也自洽(早期选帧行为区分不出赢家输家)。

预注册判停是 **60 迭代**,当前 20 迭代**不触发停止**;是否续跑到 60 是
算力取舍问题,判据本身不因这次结果改动。
...
- 协议:held-out 120(从未参与任何开发环节)× 双臂各 1 轮 × τ=0 部署
  ...
  显著性**(12:6/18 翻转,双侧精确符号检验 p=0.238,bootstrap 95% CI
  [-1.7%, +11.7%] 跨零;此前口头报的 p≈0.12 是单侧值,应以双侧为准,
