...
reg "$C" "$G" "sglang-omni-rl history-harm:格式解耦 pickimg/hybrid(port $PORT);⚠ 在用勿删;收尾:出数即删"
...
SPECS="rec2_deploy,pickimg:judge_glm46v|direct|six,pickimg:qwen38_27b|direct|six,pickimg:judge|direct|six,hybrid:judge_glm46v|direct|six,hybrid:qwen38_27b|direct|six,hybrid:judge|direct|six,hybrid:judge_glm46v|direct|full"
python3 /data01/jaxan/decode_ctx.py --base-url $E --tag base_format --specs "$SPECS" --picks "$R/picks_*.jsonl" --out $H/format_base.jsonl --workers 12 > $H/format_base.log 2>&1
echo "$(date -u +%FT%TZ) done: $(wc -l < $H/format_base.jsonl)"; rm_own "$C"; echo FORMAT_DONE
