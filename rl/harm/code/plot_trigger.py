...
H = "/data01/jaxan/harm"
files = ["harm_vs_n_base_deploy.jsonl", "format_base.jsonl", "format2_base.jsonl", "term_interv_base.jsonl", "goal_interv_base.jsonl", "reply_pattern_base.jsonl", "reply_pattern2_base.jsonl", "dose_base.jsonl"]
recs = {}
for f in files:
    p = f"{H}/{f}"
...
# 左:剂量曲线
for kind, lab, c in (("grayturnin", "M gray images (content-free, same tokens as a screenshot)", "tab:orange"), ("longtextturnin", "M text blocks (~1 screenshot worth of tokens)", "tab:green")):
    xs, ys, lo, hi = [], [], [], []
    for m in (1, 2, 3, 4):
        r = stat(f"{kind}{m}")
...
ax1.set_xlabel("M reference turns inserted after the instruction, before the last two turns"); ax1.set_ylabel("terminate rate (%)"); ax1.set_xticks([1, 2, 3, 4])
ax1.set_title("dose: what after the instruction triggers termination", fontsize=9); ax1.grid(alpha=.3); ax1.legend(fontsize=7)
# 右:5 张图的各种放法
...
for sp, lab in conds + [(f"{k}{m}", f"{k}{m}") for k in ("grayturnin", "longtextturnin") for m in (1, 2, 3, 4)]:
    r = stat(sp)
