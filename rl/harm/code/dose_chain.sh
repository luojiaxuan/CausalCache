#!/usr/bin/env bash
# note (luojiaxuan): 终止触发物剂量曲线(灰图/长文本参考轮 ×M)——rec4/rec6 的早期回复换 "Noted"(noted),参考轮插到指令之后(hybridturnin);337 态,1 张卡。
set -uo pipefail
...
reg "$C" "$G" "sglang-omni-rl history-harm:终止触发物剂量曲线(灰图/长文本参考轮 ×M)(port $PORT);⚠ 在用勿删;收尾:出数即删"
...
python3 /data01/jaxan/decode_ctx.py --base-url $E --tag base_dose --specs "grayturnin1,grayturnin2,grayturnin3,grayturnin4,longtextturnin2,longtextturnin4" --out $H/dose_base.jsonl --workers 8 > $H/dose_base.log 2>&1
echo "$(date -u +%FT%TZ) done: $(wc -l < $H/dose_base.jsonl)"; rm_own "$C"; echo DOSE_DONE
