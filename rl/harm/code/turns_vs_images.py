...
def shape(sp):
    ...
    m3 = re.match(r"(graysmallturnin|graytinyturnin|graypairturnin)(\d+)$", sp)
    if m3:
        d = int(m3.group(2)); k = m3.group(1)
        return (2, 3 + d, {"graysmallturnin": f"{sp}: {d} 个参考轮,各半张高灰图,插在指令之后", "graytinyturnin": f"{sp}: {d} 个参考轮,各 0.2 张高灰图,插在指令之后", "graypairturnin": f"{sp}: 1 个参考轮里放 {d} 张灰图(图块 {d},轮 1),插在指令之后"}[k])
