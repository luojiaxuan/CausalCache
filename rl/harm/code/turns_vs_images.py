...
files = {"deploy": "/data01/jaxan/harm/harm_vs_n_base_deploy.jsonl", "format": "/data01/jaxan/harm/format_base.jsonl",
         "format2": "/data01/jaxan/harm/format2_base.jsonl", "term": "/data01/jaxan/harm/term_interv_base.jsonl", "goal": "/data01/jaxan/harm/goal_interv_base.jsonl", "replypat": "/data01/jaxan/harm/reply_pattern_base.jsonl"}
recs = {}
...
def shape(sp):
    ...
    return {"pickimg": (2, 3, "pickimg 2轮3图(图换老帧)"), "hybrid": (2, 5, "hybrid 2轮+2标记图放首条"), "hybridlast": (2, 5, "hybridlast 2轮+2标记图放末条"),
            "hybridturn": (2, 5, "hybridturn 2轮+2参考轮(Noted 回复,插在指令之前)"), "hybridturnin": (2, 5, "hybridturnin 2轮+2参考轮(Noted 回复,插在指令之后)")}.get(kind, (None, None, sp))
rows = {}
