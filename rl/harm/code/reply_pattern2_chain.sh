#!/usr/bin/env bash
# note (luojiaxuan): 终止触发物分辨 2(灰图/纯文本参考轮)——rec4/rec6 的早期回复换 "Noted"(noted),参考轮插到指令之后(hybridturnin);337 态,1 张卡。
set -uo pipefail
...
reg "$C" "$G" "sglang-omni-rl history-harm:终止触发物分辨 2(灰图/纯文本参考轮)(port $PORT);⚠ 在用勿删;收尾:出数即删"
...
python3 /data01/jaxan/decode_ctx.py --base-url $E --tag base_replypat2 --specs "hybridturnin_gray:judge_glm46v|direct|six,hybridturnin_text:judge_glm46v|direct|six" --picks "$R/picks_*.jsonl" --out $H/reply_pattern2_base.jsonl --workers 8 > $H/reply_pattern2_base.log 2>&1
echo "$(date -u +%FT%TZ) done: $(wc -l < $H/reply_pattern2_base.jsonl)"; rm_own "$C"; echo REPLY_PATTERN2_DONE
