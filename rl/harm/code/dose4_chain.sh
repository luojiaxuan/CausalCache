#!/usr/bin/env bash
# note (luojiaxuan): 终止触发物剂量 4(禁 terminate 重跑,修正 token 集)——rec4/rec6 的早期回复换 "Noted"(noted),参考轮插到指令之后(hybridturnin);337 态,1 张卡。
set -uo pipefail
source /data01/jaxan/cc_container_lib.sh
H=/data01/jaxan/harm; R=/data01/jaxan/rl_v2; C=""
until grep -q "DOSE3_DONE\|SERVER_FAIL\|RUN_FAIL" /data01/jaxan/dose3_chain.log 2>/dev/null; do sleep 60; done
...
reg "$C" "$G" "sglang-omni-rl history-harm:终止触发物剂量 4(禁 terminate 重跑,修正 token 集)(port $PORT);⚠ 在用勿删;收尾:出数即删"
...
python3 /data01/jaxan/decode_ctx.py --base-url $E --tag base_dose4 --specs "rec4_deploy_noterm,rec6_deploy_noterm,grayturnin2_noterm,hybridturnin:judge_glm46v|direct|six_noterm" --picks "$R/picks_*.jsonl" --out $H/dose4_base.jsonl --workers 8 > $H/dose4_base.log 2>&1
echo "$(date -u +%FT%TZ) done: $(wc -l < $H/dose4_base.jsonl)"; rm_own "$C"; echo DOSE4_DONE
