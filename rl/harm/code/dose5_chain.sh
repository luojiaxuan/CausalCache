#!/usr/bin/env bash
# note (luojiaxuan): 终止触发物剂量 5(半张/1.5/2.5 张灰图细化阈值)——rec4/rec6 的早期回复换 "Noted"(noted),参考轮插到指令之后(hybridturnin);337 态,1 张卡。
set -uo pipefail
source /data01/jaxan/cc_container_lib.sh
H=/data01/jaxan/harm; R=/data01/jaxan/rl_v2; C=""
until grep -q "DOSE4_DONE\|SERVER_FAIL\|RUN_FAIL" /data01/jaxan/dose4_chain.log 2>/dev/null; do sleep 60; done
...
reg "$C" "$G" "sglang-omni-rl history-harm:终止触发物剂量 5(半张/1.5/2.5 张灰图细化阈值)(port $PORT);⚠ 在用勿删;收尾:出数即删"
...
python3 /data01/jaxan/decode_ctx.py --base-url $E --tag base_dose5 --specs "grayfracturnin05,grayfracturnin15,grayfracturnin25" --out $H/dose5_base.jsonl --workers 8 > $H/dose5_base.log 2>&1
echo "$(date -u +%FT%TZ) done: $(wc -l < $H/dose5_base.jsonl)"; rm_own "$C"; echo DOSE5_DONE
