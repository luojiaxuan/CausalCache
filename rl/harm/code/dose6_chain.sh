#!/usr/bin/env bash
# note (luojiaxuan): 终止触发物剂量 6(小图块 / 同轮多图:块的定义)——rec4/rec6 的早期回复换 "Noted"(noted),参考轮插到指令之后(hybridturnin);337 态,1 张卡。
set -uo pipefail
source /data01/jaxan/cc_container_lib.sh
H=/data01/jaxan/harm; R=/data01/jaxan/rl_v2; C=""
until grep -q "DOSE5_DONE\|SERVER_FAIL\|RUN_FAIL" /data01/jaxan/dose5_chain.log 2>/dev/null; do sleep 60; done
...
reg "$C" "$G" "sglang-omni-rl history-harm:终止触发物剂量 6(小图块 / 同轮多图:块的定义)(port $PORT);⚠ 在用勿删;收尾:出数即删"
...
python3 /data01/jaxan/decode_ctx.py --base-url $E --tag base_dose6 --specs "graysmallturnin2,graysmallturnin3,graytinyturnin2,graytinyturnin3,graypairturnin2,graypairturnin3" --out $H/dose6_base.jsonl --workers 8 > $H/dose6_base.log 2>&1
echo "$(date -u +%FT%TZ) done: $(wc -l < $H/dose6_base.jsonl)"; rm_own "$C"; echo DOSE6_DONE
