...
# 第四面板:干预(rec2/rec4 的 PAST 标记版与灰图版);第五面板:UI-Venus-2-9B 的 N 扫描(跨模型)。误差棒 = 按状态 bootstrap 95% CI。
...
ax = axes[4]; rows = load(f"{H}/harm_vs_n_venus_text.jsonl")
if rows:
    VN = [0, 1, 2, 3, 4, 6, 8]
    S = series(rows, [f"rec{n}" for n in VN] + ["irr2", "irr4", "irr8"]); summary["venus"] = S
    for kind, lab, c in (("rec", "real most-recent N frames", "tab:blue"), ("irr", "N frames from another task", "tab:gray")):
        xs = [n for n in VN if f"{kind}{n}" in S or (kind == "irr" and n == 0 and "rec0" in S)]
        ys, lo, hi = [], [], []
        for n in xs:
            k = "rec0" if (kind == "irr" and n == 0) else f"{kind}{n}"; m, l, h, _ = S[k]; ys.append(100*m); lo.append(100*(m-l)); hi.append(100*(h-m))
        ax.errorbar(xs, ys, yerr=[lo, hi], marker="o", capsize=3, label=lab, color=c)
    ax.set_xlabel("number of history screenshots N"); ax.set_ylabel("action reproduction (%)"); ax.set_title(f"frozen UI-Venus-2-9B · native multi-turn layout, text kept · n={len(rows)}", fontsize=9); ax.grid(alpha=.3); ax.legend(fontsize=8)
