...
- 剂量曲线是**阶跃**:指令之后 1 张灰图无事(92.6),2 张就塌(72.4,终止 20.8%),3–4 张不再加深;合上 rec3 不塌 / rec4 塌,
  ...
  文本长度只贡献一个小的渐变距离效应。同剂量放**指令之前**:1–4 张灰图都无害(90.5–92.3,终止 ≤0.9%)——放置是全部。禁 terminate 解码回收约六成差距
  (rec4 70 → 84,初轮 token 集有误、重跑中);拼图对照定案:2–4 张灰图拼成**一块**只 3–4% 终止(分开放 16–21%),token 与位置位移相同——**变量是图块数,不是视觉 token 量**。
