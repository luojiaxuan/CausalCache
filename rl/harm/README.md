...
| 历史图数 N | **部署布局**(交错多轮,agent 真实所见) 真实 / 无关 | 标注器布局(图连排在前 + 全部 conclusion) 真实 / 无关 |
|---|---|---|
| 0 | 71.8 | 62.9 |
...
1. **部署布局下历史图先益后害**:N=1–3 把复现率从 71.8 抬到 93.5,N≥4 骤降到 70 以下。骤降的形态是**过早终止**——N=4/6 的错误里
   73–74% 输出 "Terminate, task finished",而目标动作无一是 terminate。长的交错历史让模型误以为任务已完成(机制 A,H5)。
...
跨模型:UI-Venus-2-9B 去掉文本轨迹后,裁判选的老帧比无关帧多救回 +20~+31pp(九格 CI 全不含 0),比最近两帧好 17–27pp;保留文本轨迹时
同方向、幅度小。视觉历史该取哪几张,按模型与协议说。
...
**Step 3 机制假设 + 干预检验**(H2 已获支持、H3 已排除、H5 新增;剩余干预针对 H5)——候选假设——
