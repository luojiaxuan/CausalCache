...
docker run -d --name sglang-omni-jaxan-rl0 --init --gpus '"device=0"' --network host \
...
printf "sglang-omni-jaxan-rl0\tgpus=0\thost=hyper00\tcreated=%s\tdesc=sglang-omni-rl smoke;收尾:smoke 完成即删\n" \
...
cd "$MW"
CC_SEL_DEVICE=cuda:1 CC_SEL_PORT=41010 CC_SEL_LOG="$D/decisions" \
...
for it in $(seq 0 $((ITERS-1))); do
  ...
  cd "$MW" && CUDA_VISIBLE_DEVICES=1 timeout 3600 uv run --with peft,pillow \
