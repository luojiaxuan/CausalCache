# note (luojiaxuan): selector 推理服务(smoke)。rollout 期挂在 127.0.0.1,
# MobileWorld agent 的 learned 策略每步 POST /select。
# 每个决策落 JSONL(训练数据面);失败即 500 —— 不做 recent 回退,
...
from selector import FrameSelector, TinyFrameEncoder

DEVICE = os.environ.get("CC_SEL_DEVICE", "cuda:0")
LOG_DIR = os.environ.get("CC_SEL_LOG", "/data/grpo/decisions")
...
def _feats(b64_list):
    ...
    for b in b64_list:
        ...
        ims.append(torch.tensor(list(im.getdata()), dtype=torch.float32).view(1, 64, 64) / 255.0)
    with torch.no_grad():
    ...
    def do_POST(self):
        try:
            ...
            if self.path == "/reload":
                ...
                # note (luojiaxuan): feats 一并落盘 —— 训练期要在当前参数下重算
                # slate logprob(比率裁剪),编码器冻结故特征可复用。
                rec = {
                ...
                with open(f"{LOG_DIR}/{rec['task']}.jsonl", "a") as f:
