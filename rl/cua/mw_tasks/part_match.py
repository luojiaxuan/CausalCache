...
An 'approved sample' photo is shown once (Download/Approved). Later the agent must pick, among four candidate
...
before the choice are evidence-free by construction. The Files app renders image folders as a two-column grid
that shows two full rows with file names; four candidates therefore fit without scrolling, so the choice is a
single click whose grid cell identifies the file. The parts differ only in shape, colour and hole layout — no text — so the text
...
class _PartMatchMixin:
    ...
    goal = (
        ...
        "Then look through the four photos in Documents/Candidates and copy the ONE photo that shows the same part "
