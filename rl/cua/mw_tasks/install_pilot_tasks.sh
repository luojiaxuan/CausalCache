...
for c in "$@"; do
  port=$(docker inspect "$c" --format '{{range $p,$v := .NetworkSettings.Ports}}{{$p}} {{end}}' | grep -o '68[0-9][0-9]' | head -1)
  ...
  ok=$(curl -s -m 10 "http://127.0.0.1:${port:-6800}/task/goal?task_name=QuoteRecallTaskA" | head -c 80)
  echo "$c (port ${port:-?}): $([ -n "$ok" ] && echo "OK: $ok" || echo "NOT REGISTERED")"
done
