#!/usr/bin/env bash
# note (luojiaxuan): PartMatch 第四版(四候选,2×2 网格全部可见)全链:等 v3 链结束释放模拟器 → 装任务类 →
...
until grep -q PILOT_PM3_DONE /data01/jaxan/pilot_pm3_chain.log 2>/dev/null; do sleep 60; done
...
arm() {  # $1 agent_type $2 model path $3 served $4 backend $5 out prefix dir $6 extra env $7 protos
  ...
  reg "$C" "$G" "sglang-omni-rl B-pilot PartMatch v4 采集+矩阵($3);⚠ 在用勿删;收尾:出数即删"
...
arm ui_venus2 /data01/jaxan/models/UI-Venus-2-9b UI-Venus-2 venus $P/prefix_venus_pm4 CC_VENUS_HIST=recent:2 "text notext"
arm gui_owl_1_5 /data04/jaxan/models/GUI-Owl-1.5-8B-Instruct gui-owl owl $P/prefix_base_pm4 "CC_HISTORY_N=3 CC_FRAME_POLICY=recent" "text"
echo PILOT_PM4_DONE
