...
class GUIOWL15AgentMCP(MCPAgent):
    def __init__(
        ...
        **kwargs,
        ...
        # note (luojiaxuan): 预算 B = history_n - 1,由 CC_HISTORY_N 注入。
        self.history_n = int(
    ...
    def predict(
        ...
        _dump = __import__("os").environ.get("CC_DUMP_DIR")
        if _dump:
            import json as _j
        ...
        # note (luojiaxuan): CausalCache 干预面 —— 由策略决定保留哪些帧为图像。
        # CC_FRAME_POLICY=recent 时 _cc_S == range(text_history_count, total+1),
        # 即官方行为(parity 测试逐字节校验)。
        _cc_S = _cc_select_frames(
            total_history_count,
            keep_as_messages,
            ...
            _goal=str(getattr(self, "instruction", "")),
            _concls=(["initial screen"]
                     + [str(c) for c in self.conclusions])[:total_history_count],
            _cur_b64=(self.history_user_content[total_history_count][0]
                      if len(self.history_user_content) > total_history_count
                      else None),
        )
        logger.warning(f"CC_TRACE policy={os.environ.get(chr(39)+chr(67)+chr(67)+chr(95)+chr(70)+chr(82)+chr(65)+chr(77)+chr(69)+chr(95)+chr(80)+chr(79)+chr(76)+chr(73)+chr(67)+chr(89)+chr(39))} hist_n={self.history_n} total={total_history_count} S={_cc_S}")
        _cc_kept = set(_cc_S)
        _cc_text_idx = [i for i in range(total_history_count) if i not in _cc_kept]
        ...
        if _cc_text_idx:
            previous_steps_text = _cc_format_steps_by_index(self, _cc_text_idx)
        ...
        first_img_encoded, _, _ = self.history_user_content[_cc_S[0]]
        ...
        # 对 S 中相邻的两帧,插入前一帧所属 turn 的 assistant 回复。
        for _k in range(len(_cc_S) - 1):
            _turn = _cc_S[_k]
            history_resp = self.history_responses[_turn]
            messages.append(
                {
                    "role": "assistant",
...
# note (luojiaxuan): CausalCache 帧选择策略。返回保留为图像的索引集合 S,
# 元素取值 0..total(total 为当前观察),恒含 total。budget = 历史帧数 B。
def _cc_select_frames(total, budget, policy=None, step=0, salt="", _frames_b64=None, _task="", _episode="", _goal="", _concls=None, _cur_b64=None):
    import os as _os
    import random as _random

    policy = policy or _os.environ.get("CC_FRAME_POLICY", "recent")
    budget = max(0, min(budget, total))
    if policy == "recent":
        # 与官方等价:连续尾窗
        return list(range(total - budget, total + 1))
    if policy == "random":
        import hashlib as _hashlib

        seed = _os.environ.get("CC_SEED", "0")
        # note (luojiaxuan): 种子必须含**任务标识**,否则同一 total 下所有任务
        # 抽到同一组帧(旧 bug)。用 hashlib 而非内置 hash:后者受 PYTHONHASHSEED
        # 随机化,跨进程不可复现。
        key = f"{seed}|{salt}|{step}|{total}".encode("utf-8")
        rng = _random.Random(int(_hashlib.md5(key).hexdigest()[:16], 16))
    ...
    if policy == "learned":
        ...
        payload = _json.dumps({
            ...
            "goal": _goal, "concls": _concls,
            "cand_steps": list(range(1, len(_frames_b64 or []) + 1)),
            "cur_b64": _cur_b64,
            "tag": _os.environ.get("CC_EP_TAG", ""),
        }).encode()
...
def _cc_format_steps_by_index(agent, indices):
    """按任意索引集合渲染文本历史(官方 _format_previous_steps 仅支持连续区间)。"""
    previous_steps = []
    for i in indices:
        step_num = i + 1
        conclusion = add_period_robustly(agent.conclusions[i])
        step_info = f"Step{step_num}: {conclusion}"
        tool_call_res = (
            agent.history_user_content[i][1]
            if i < len(agent.history_user_content)
            else None
        )
        ask_user_res = (
            agent.history_user_content[i][2]
            if i < len(agent.history_user_content)
            else None
        )
        if tool_call_res:
            step_info += f"  Tool response: {tool_call_res}"
        if ask_user_res:
            step_info += f"  User response: {ask_user_res}"
        previous_steps.append(step_info)
    return "\n".join(previous_steps)
