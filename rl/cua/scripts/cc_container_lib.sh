...
run_labeled() { local i err; for i in 1 2 3; do C=$(alloc_name); if err=$(docker run -d --init --label cc.owner=$OWN --name "$C" "$@" 2>&1 >/dev/null); then return 0; fi; echo "run_labeled attempt $i ($C): ${err:0:200}" >&2; docker rm -f "$C" >/dev/null 2>&1; sleep $((RANDOM % 5 + 1)); done; C=""; return 1; }
pick_port() { local used p; used=$(docker ps -a --format '{{.Ports}}' | grep -o '172\.17\.0\.1:[0-9]\+\|0\.0\.0\.0:[0-9]\+' | sed 's/.*://' | sort -u); for p in "$@"; do grep -qx "$p" <<<"$used" || ss -ltn 2>/dev/null | grep -q ":$p " || { echo "$p"; return; }; done; }
# docker run 失败会留下 Created 状态的同名容器(端口冲突等);只清理标签等于本次 OWN 的
rm_created_own() { local c; for c in $(docker ps -a --filter status=created --format '{{.Names}}' | grep '^sglang-omni-jaxan-'); do [ "$(docker inspect -f '{{index .Config.Labels "cc.owner"}}' "$c" 2>/dev/null)" = "$OWN" ] && docker rm -f "$c" >/dev/null 2>&1 && echo "removed created-only $c"; done; }
