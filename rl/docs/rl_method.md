...
| ~~离线 U 增益不迁移闭环~~ **(2026-08-08 更正,见下)** | 训练信号**就是**闭环成败 |

> **对"离线增益不迁移闭环"这一条的更正(2026-08-08,用户指出)**:
> 这个框架是错的。`indomain_gap_v1` 的实测是 **模型−锚 = +0.0066**、
> **oracle−锚 = +0.1019**、**捕获率 6.5%**——我们的 selector 离线只拿到了
> 可得头寸的十五分之一,这个量级**不足以改变动作**,闭环没反应是应然而非
> 反常。加之离线语料(AgentNet/OpenCUA 桌面示范)与闭环(OSWorld 任务)
> 本就不同源,口径也不可直接互推。
> **正确的表述是:我们从未产生过足够大的离线增益,因此"离线→闭环迁移"
> 这个命题至今没有被真正检验过。** 这与"迁移被证伪"是两回事。
