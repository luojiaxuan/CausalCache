...
## 4.12 批 30 对照点(2026-08-27)——测量管线通,推断留给交叉协议

**编排**:recent 对照批零额外 GPU 方案落地——官方 runner + 池 32 台 +
**在跑训练引擎的 /v1 兼容端**作策略服务(读数跨 1-2 次权重热换的邻域
平均,ARM 文件记录);20 题 train 抽样(seed=20260827),recent-B2 口径,
并发 8,主跑无感。

**读数**:recent @ 当前权重 = 6/20 = **30.0%**(vs P0.5 基础权重
recent-B2 ≈31%:executor 经 30 批 learned 训练后 recent 能力无退化迹象);
learned 训练内同 20 题累计 164/548 ≈ **29.9%**——总量打平;方向面
learned 10 高 / recent 5 高 / 5 平,learned 强项在 recent 全灭题
(AdjustFontIcon 80%、GoogleMapsSouthNeighbor 55%),但后者在 QA 记忆化
名单,**不作学习证据**。

**混杂声明(为何这不是判决)**:τ=1.0 训练采样 vs 官方评测口径;跨 31
批策略演化混合;逐题 n 从 7 到 55 不等;对照单次 0/1。定位=管线验证 +
"无崩坏/无碾压"的粗信号;推断归 checkpoint 交叉评测(τ=0,{E_0,E_t}×
...
## 4.14 全量训练收官(2026-08-29):91/100 批,引擎失联提前终止

**终止形态**:第 91 批权重热换时,rollout 引擎 `172.17.0.6:15002` 的
`/pause_generation` 连接被拒 → 主进程 exit 1。**根因可回溯到 8-27**:
该 worker 的 `/generate` 从两天前就间歇失败,sgl-router 自动重试掩盖了
症状,直到它彻底死亡才暴露。

**监控缺口与规则补充(向前)**:守夜过滤器只盯 `grpo.py:183` 批线、
OOM、Traceback 与致命签名,**router 层的 worker 错误率不在其中**。
今后长跑必须把 `smg::routers::.*Failed to send typed request` 的
**频次**纳入监控(单 worker 每小时 >3 次即告警),否则"引擎正在慢性
死亡"这类形态会一直隐身到崩溃。

**处置与决策**:最终权重 `hf_p2/iter_86`(全局第 89 批,17G 完整,
29 个 checkpoint 齐)。**决策:不补跑剩余 9 批**——近十批 rate 均值
0.45-0.5 已平台化,9 批的边际信息量小于重启成本(Ray 冷启 20 分钟 +
5 小时 + 面对同一不稳引擎);判决评测才是关键路径。**可推翻**:
iter_86 在盘,随时可从它续跑补齐或延长。

**全程曲线(91 批,含混杂声明)**:批均 rate 由前 8 批的 0.305 升至
末段十批的 ~0.46;混合组率带 25-88%(均值 ~57%,优先采样维持信号
密度);批墙钟 31-40 分(四卡双引擎 conc64);episode 损耗率 ~10%
(批边界容器风暴);容器内存 307-517G(offload 稳态,未近 1050g 线)。
**两列纪律照旧**:该曲线含 QA 记忆化通道(12/78 题走文本子串评测),
真实增益以判决评测(§4.15)为准。

## 4.15 ★★ 全量终判(2026-08-29):heldout 上 learned 领先 recency
+8.8pp,方向一致但**未达统计显著**;不构成胜出声明

**协议**:最终权重 `hf_p2/iter_86`(91 批训练的产物)× 五格 × τ=0 ×
官方判分;heldout-39 **全程未训练**(过拟合免疫),train-20 与
§4.12/4.13 同集可纵向比。独立 vLLM(GPU0),池 32 台。

### heldout-39(判决集,共同判分 34 题;QA 12 题恰好全不在 heldout,
故全域=态验证域)

| 格 | 成功 | 率 |
|---|---|---|
| **E_t × S_t(learned)** | 12/34 | **35.3%** |
| E_t × recency | 9/34 | 26.5% |
| E_t × S₀(初始 selector) | 9/34 | 26.5% |

逐题配对与符号检验:
- **S_t vs recency:5 胜 / 2 负 / 27 平,p=0.453**
- S_t vs S₀:4 胜 / 1 负 / 29 平,p=0.375
- recency vs S₀:3 胜 / 3 负 / 28 平,p=1.000

### train-20(同 §4.13 集,可纵向比)

态验证 14 题:S_t 28.6% vs recency 42.9%(0 胜 / 2 负 / 12 平,p=0.5)。

### 判读(三条,含反向证据)

1. **主判据方向转正但不显著**:heldout 上 learned 比 recency 高
   **+8.8pp**,**超过预登记 MDE(+3pp)**,逐题 5:2 同向;但符号检验
   **p=0.453 远未达显著**,有效样本仅 7 题(27 题两臂同分)。
   **诚实结论:不能宣称 learned 胜出,只能说"heldout 上未发现劣势,
   且方向有利"。**
2. **与批 60 诊断(§4.13)方向相反,须并列报告**:批 60 在 train-20 上
   是 learned 40% < recency 45%;本次 heldout 上是 35.3% > 26.5%,而
   同一张 train-20 上仍是 28.6% < 42.9%。**train 与 heldout 的方向系统
   性相反**——最可能的解释是 train 域上 recency 因 60+ 批的 on-policy
   训练获得了额外适配(executor 在 recency 分布上被反复强化),而
   heldout 上这种适配不可迁移,两臂回到较公平的比较。这个解释本身
   **未经检验**,记为待验假设,不作结论。
3. **selector 学习的证据弱化**:S_t vs S₀ 在 heldout 上 4:1(p=0.375),
   量级与 §4.13 的 +10pp 同向但样本更弱;recency 与 S₀ 打平
   (3:3)——说明"训练后的 selector ≈ 初始 selector ≈ recency"这一
   零假设在当前样本量下无法排除。

### 判停线执行(预注册 §6)

MDE 达成但显著性未达 → **不触发"科学负结果"回退声明,也不构成胜出**。
按预注册口径,此为**证据不足**,需要更大样本或更长训练:
- 立即可做:heldout-39 多次重复(τ=0 下环境仍有随机性)把 27 个平局
  拆开;成本 ~每轮 1.5h/格;
- 分诊后手:§4.13 的 random-S SFT 料已备(11190 样本,`build_random_s_
  sft.py`),若续期训练则先 SFT 再 RL;
- 续期:iter_86 可直接续跑(恢复链已验证 3 次)。

## 4.16 ★★ selector 优化病理三连诊断与修复(2026-08-30/31)

外审(p3_diagnosis_review_20260830.md,极高档)给的执行顺序是"先仪表化
再干预、一次只动一项"。照做,**当场挖出两个真 bug,clip_frac 从长期
0.55 降到 0.165**(正常 PPO 区间),链条完全闭合:

| # | 改动 | 诊断依据 | clip_frac |
|---|---|---|---|
| — | 起点(长期观测) | — | ~0.55 |
| ① | **PL 有序/无序记账修复** | 重放自检:同权重零优化下 **68.3% 决策 ratio≠1**、57.3% 越界 | 0.56(不变) |
| ② | **轮内优化步数 27→8**(minibatch 自适应) | 逐步追踪:step0 **0.0%**、1步 11.5%、5步 18.2% → 外推 27 步即 50%+ | **0.426** |
| ③ | **丢弃 >15 分钟陈旧决策** | 年龄分桶:当前权重下产生的 1923 条 **0.0%**,3-6 分 31.6%,>15 分 12.7% | **0.165** |

### ① PL 记账 bug(根因,外审点名"要非常早查"的那条)

`FrameSelector.sample()` 返回 `sorted(idx)`,而它累加的 `logp` 是按
**实际抽取顺序**算的;训练期 `slate_logprob()` 又按那个排序后的顺序重算。
**Plackett-Luce 是有序过程**:P(先a后b)=p_a·p_b/(1−p_a) ≠ P(先b后a)。
后果:**策略权重完全没变,重要性比率就已经不等于 1**。实测有序/无序
logprob 差中位 0.587(ratio 差 1.8 倍)。

修复:`sample` 返回采样顺序(渲染侧无需有序——
`protocol._select_messages` 自己 `sorted(set(S))`)。修复后自检
0/300 失配、ratio 恒 1.0000。加两个回归测试(采样-重算一致性、
PL 顺序敏感性)。

**影响面(必须记账)**:P1-P3 共 133 批的 selector 训练中,**超过一半的
梯度被裁掉的是记账误差而非真实策略更新**——这解释了 recency_mass 长期
震荡不收敛。历史 decisions 的 `chosen` 已被 sorted 破坏且原始采样顺序
**不可恢复**,74 个决策文件全部归档作废;selector 权重保留继续训练
(heldout 上仍有 +8.8pp,非随机),污染事实在此存档。

### ② 轮内累积漂移(主因)

固定 MB=64 时,一轮 ~1700 条决策要走 ~27 个优化步;逐步追踪显示单步
幅度正常(11.5%)但累积可观。改 `MB=max(64, ⌈n/8⌉)`:**大批少步,
数据用满而漂移可控**。

### ③ 行为策略陈旧(次因)

年龄分桶证明记账修复彻底(当前权重下的决策 0.0% 越界),同时量出
staleness 的真实贡献。`--max-age 900` 丢弃陈旧决策(实测一轮丢 1180 条),
新数据每分钟都在产,丢弃比让裁剪吃掉梯度划算。

### 方法学教训

**① 单独看"无效"(clip_frac 没变),但它是 ②③ 可解释的前提**——不修它,
比率里混着与策略变化无关的记账误差,后两项的读数根本无法归因。
...
## 4.18 ★★ selector-only 评测(外审第 4 步,2026-08-31)——**负面结果**

**协议**:executor **冻结**在 `hf_p3/iter_50`(全局第 141 批),三臂只换
选帧策略;独立 selector service(:41011)+ 冻结权重快照,与训练中的
:41010 完全隔离(上次 xplay 临时 reload 训练服务带偏了主跑 2 批);
heldout-39 × **2 seed** 提功效;GPU5 独立 vLLM,不占训练卡。

### 主实验(共同判分 32 题 × 2 次 = 64 次)

| 臂 | 成功 | 率 |
|---|---|---|
| E × S_t(learned) | 17/64 | **26.6%** |
| E × recency | 21/64 | **32.8%** |
| E × S₀(初始 selector) | 18/64 | 28.1% |

逐题配对(态验证域 n=32;QA 12 题恰好全不在 heldout):
- learned vs recency:3 胜 / 5 负 / 24 平,**p=0.727**
- learned vs S₀:4 胜 / 5 负 / 23 平,p=1.000
- recency vs S₀:6 胜 / 3 负 / 23 平,p=0.508

**效应量:learned − recency = −6.2pp,95%CI [−20.0, +7.5]pp**(预登记
MDE=+3pp)。

### 判读:与 §4.15 方向相反,合并看效应在零附近

| 评测 | checkpoint | 口径 | learned − recency |
|---|---|---|---|
| §4.15 批 100 终判 | iter_86(全局 89 批) | 单次,n=34 | **+8.8pp**(p=0.45) |
| §4.18 本次 | iter_50(全局 141 批) | 2 seed,n=32 | **−6.2pp**(p=0.727) |

**两次符号相反、两次都不显著**。最简洁的解释是**真实效应接近零**,
先前的 +8.8pp 属噪声(当时已声明 CI ±0.17、有效样本仅 7 题)。
本次 CI 仍宽(±14pp),同样无法建立等价性——但**"learned 显著优于
recency"这一主张,在两次独立评测中均未获支持**。

另一读法:S₀(28.1%)夹在 learned(26.6%)与 recency(32.8%)之间,
**三臂互无显著差异**——即 130+ 批训练使 selector 行为大幅改变
(recency_mass 0.57→0.26)却未转化为任何可测的性能变化。

### ★ 意外发现:非法动作被训练侧静默吞掉

诊断格挖出 `CheckInvoiceTask3` 全败的**真因,且与 selector 无关**:
> `ValueError: Unknown action_type: 'double_tap'`(重试 10 次全部同因)

评测日志共 **157 次**非法动作(`double_tap` 78、`'{` 79、`key` 1),
**训练日志 0 次**。原因是两条通路对未知动作的处理不同:
- **训练侧**(我方 `action_space.py`)走 `unknown_wrapper_action`,
  把未知动作**静默转成 unknown 反馈**,episode 继续;
- **评测侧**(官方 runner)直接抛 `ValueError`,任务判失败。

后果:**模型生成 double_tap 从未在训练中受到明确惩罚**,因而持续生成;
在训练里表现为"动作无效→任务失败"(CheckInvoiceTask3 的 37 次全败),
在官方评测里表现为直接崩。**这是动作空间不匹配 + 静默吞并的复合缺陷,
不是 selector 的问题,也不是能力问题。**

非法动作在三臂间基本均匀(每臂 22-24 次,唯 recency_s2 仅 2 次),
故不构成 −6.2pp 的解释;但它系统性拉低了所有臂的绝对成功率。

**待办(下一阶段)**:(a) 让非法动作在训练中可见——至少计数上报,
考虑显式负反馈;(b) 核对 GUI-Owl-1.5 原生动作空间与 MobileWorld
mobile_use 方言的差集,double_tap 属模型先验带入;(c) 查那 79 次
`'{` 的解析路径(疑似嵌套 JSON 未拆)。
...
## 4.19 决策日志:selector 决策过滤改按策略版本(2026-09-01,在跑中部署)

**问题(本该问 luojiaxuan)**:P4 在跑,是否中途修改 selector trainer 的
数据过滤判据?改动会在实验中间引入不连续。

**我选的默认**:改,并当场部署到 P4。

**理由(实测)**:`--max-age 900` 秒**短于**实测轮次周期(163 次 reload,
中位间隔 2178 秒)。而权重热换只在轮次边界发生,一轮窗口内的决策全部出自
同一策略版本、对本轮起始权重完全 on-policy。按墙钟丢弃的后果是
`episodes_trained` 仅 11–23 / `episodes_with_adv` 56–62;实测丢弃率 56%,
与"均匀到达"预测的 (2178−900)/2178 ≈ 59% 吻合,证明丢弃完全由窗口内墙钟
年龄解释,而非真实的策略陈旧。**约 2/3 的同版本数据被白扔**,直接削弱了
我们正想观测的"selector 是否在学"。

**如何回滚**:旧版 `trainer.py` / `service.py` 备份在
`/data01/jaxan/sglang-omni-rl/*.py.bak`;部署脚本 `deploy_pv.sh` 内置
service 就绪失败时的自动回滚。手工回滚 = 拷回 .bak + 重启两个 sidecar
(去掉 `CC_SEL_PV_FILE`,加回 `--max-age 900`)。Git 侧 revert 提交
`77fc51b` 即可。

**外审说了什么(全文见 `docs/reviews/selector_version_filter_review_20260901.md`)**:
同意主结论——"900 秒规则对 policy staleness 是错误的抽象,版本过滤才是正确
的主机制"。但指出我"exactly on-policy"说过头,给了三个漏洞,其中两条我采纳
并改了实现:
- **有效性单位应是 episode 而非单条决策**(跨 reload 的 episode 即使只留新
  版本决策,其状态仍由旧版本动作诱导)→ 已改为整条弃用。**先量化再部署**:
  717 条 episode 中仅 4 条(0.6%)跨边界,故这条更严格的规则免费;
- **版本号只能在服务确实加载成功之后激活**(我原实现先落盘再热换,一次失败
  的 reload 会让下一轮把全部决策判为不匹配而清零数据面)→ 已改为
  urlopen 成功后再落盘,并加回归测试锁住顺序。
另新增外审要求的 **serving/training 一致性自检** `logp_consistency_mae`:
轮次起始权重即采样时权重,故版本匹配样本的 logp 重算值应与落盘值相等;
不为零即存在版本号查不出的失配——**这正是 PL 顺序记账 bug 当初逃过检查的
通道**。未采纳:跨版本重要性采样、`(run_uuid, checkpoint_hash)` 复合版本号
(单副本单进程,收益不抵改造成本)。

**部署与验收**:01:25 重启两个 sidecar,service 12 秒就绪;新决策已带
`pv` 戳;pv 文件待首次成功 reload 后写入。训练批线在 17:09(UTC)继续,
未受影响。验收判据(外审给的)——接受率应显著跳升,而
`logp_consistency_mae` 应约等于 0;若后者不为零,说明推理与训练两侧对 PL
语义仍有分歧,需在合入结论前查清。


## 4.20 P4 停线记录与两起事故(2026-09-01)

**停线**(luojiaxuan 指令;§10 详):P4 的 selector 结构性无效(看不到当前
帧与 goal),对照价值不成立。收尾清单全项执行:看护无残留;selector
sidecar 停;ray stop 211 进程,GPU 0/1/3/4 释放;checkpoint 持久化核验后删
容器 `sglang-omni-jaxan-1` 并清 map;顺删 exited 28h 的旧命名容器
`sglang-omni-jaxan-20260830-*`;三个监控停。P4 最终态:11 批,
hf_p4/iter_11 末次保存,未评。

**保留项(决策日志)**:env 池 p00–p31(32 容器)保留——重建成本高(暖机
43h+)、宿主内存充裕(可用 1.3TB)、selector RL 数日内复用;回滚 = 随时可
停。selector 版本过滤(§4.19)因停线未获在线验证轮,单测已锁,验收判据
(接受率跳升 + logp_consistency_mae≈0)移交下一条 RL 线首轮。

**事故一:map 第三次被 read-filter-overwrite 抹空。** 本次变体是单表达式
`open(p,"w").writelines([l for l in open(p) ...])`——方法宿主对象先求值,
`open(p,"w")` 先截断,列表推导读到空文件。arms_launch 收尾先抹一次(连带
pool 32 条登记),p4_teardown 又抹一次;靠 probe2 收尾留下的全量备份 +
rebuild_map.sh(按 docker 实况重建)恢复,不变量复核 32/32。带该模式的三个
脚本已改为先读后写两语句。教训:map 重写一律走 rebuild_map.sh。

**事故二:本地 Mac 的 TCC 按进程拒读,把两个文档的空 blob 推上了 main。**
worktree 里 `wc`/`ls`/`scp` 读文件正常,而 `python`/`head`/`grep`/`tail`
读到空或报错;push 管线里 `open(path).read()` 静默拿到空串,base64 后把
0 字节 blob 提交成 44ccc3b,截断了 summary_retrieval_design 与本文档的远端
版本(代码与测试文件不受影响,已逐一核对 HEAD blob 大小)。从历史正本
(7171355 / 41f10fd)恢复并补上未落地的修正。教训:全局规则本就要求
gh api 提交后"重取 contents 比对 .size"——此前一直跳过,自本次起每次
push 后必验;本地 worktree 读写不可信期间,文档编辑一律"远端取正本 →
/tmp 修改 → push → 验 size"。
