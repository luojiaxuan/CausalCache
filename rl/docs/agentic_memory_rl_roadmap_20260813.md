...
1. Phase 3 正式训练:分层采样 + group 16,多轮迭代,评测用
   `--selector-greedy` 的 learned vs recent-2 配对(syn_ood 划分);
2. 补 Phase 2 的统计强度:闭环每臂 150 任务 + bootstrap CI(当前 n=40);
3. 闭环 OOD(syn_ood 上的四臂),补齐"不是背模板"的闭环证据。
