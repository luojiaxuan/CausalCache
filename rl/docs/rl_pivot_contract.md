...
# v3 转向:训练搬到离线视频,OSWorld 降级为评测环境(2026-08-08 用户裁定)

> **用户原话:"不要傻啦吧唧的用 OSWorld 做 RL,太慢,还得开模拟器,而且
> 奖励过于稀疏,用离线视频的每一步来做监督信号,这样训得快得多。"**
>
> **v2 于 iter-6 主动终止**(非失败终止)。终止时状态:
>
> | iter | 训练带成功 | selector_drift | 优化步 | KL |
> |---|---|---|---|---|
> | 1 | 11/33 | 0.104 | 4 | 0.212 |
> | 2 | 25/48 | 0.242 | 11 | 0.206 |
> | 3 | 24/48 | 0.141 | 8 | 0.168 |
> | 4 | 24/48 | 0.098 | 3 | 0.196 |
> | 5 | 16/48 | 0.177 | 9 | 0.243 |
>
> v2 已达成它的目的:**证明 lr 修复有效**(累计位移 0.247 vs v1 二十迭代的
> 0.044),上线门槛通过。继续跑到 20 迭代的边际价值低于转向。
> 权重与轨迹保留在 h00 `/data04/jaxan/rl2/`,可随时恢复。
>
> **转向依据(量化)**:
>
> | | OSWorld 闭环 RL(v1/v2) | 离线步级监督(v3) |
> |---|---|---|
> | 监督信号数 | 936 个(每 episode 一个 0/1) | **25,929 个**(每步一个) |
> | 信用分配 | 一个奖励背约 670 个决策 | **一个奖励背 2 个决策** |
> | 每迭代墙钟 | ~50 分钟 | 分钟级 |
> | 依赖 | QEMU 虚拟机 + KVM + docker | **纯 GPU 前向** |
>
> **OSWorld 的新定位:只做周期性闭环检验,不进训练循环。**
> 这一点不可动摇——离线步级正确率是闭环任务成功率的代理,
> 代理有效性必须靠闭环实测,且按用户裁定"要早做、小步做"。

---

# v2 预注册(2026-08-08,已于 iter-6 主动终止,见上)
...
> **判定结果(2026-08-08 07:51Z):通过。** 逐迭代 `selector_drift` =
> 0.104 / 0.242 / 0.141(门槛 0.02,超出 5-12 倍),优化步数 4 / 11 / 8;
> iter-1 `lora_b` 范数 0.079 已达 v1 iter-20(0.0399)的两倍;iter-2 相对
> 随机初始头的累计位移 0.2474、选择重合率 0.141、打分 Spearman −0.252。
> 按契约第 2 条进入"跑满 20 迭代 → 双轮 held-out 评测"。
> (iter-1 因 OSWorld 端口锁事故只收 33/48,iter-2 起恢复 48/48。)
