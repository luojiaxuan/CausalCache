#!/usr/bin/env python3
"""Validate, freeze, execute, or replay the fresh-16 claim repair."""
...
from causalcache.gate_v1_fresh16_claim_serialization_repair_runner import (
...
def _run(args: argparse.Namespace) -> Mapping[str, Any]:
    ...
    if args.command == "validate-source":
        source_only = validate_fresh16_claim_serialization_repair_source_only_contract(
            args.contract,
            repository_root=args.repository_root,
        )
        return {
    ...
    # note (luojiaxuan): Clean pushed Execution-B and every retained local byte
    # are verified before the token file is read or Hugging Face is imported.
    validate_execution_b_source(
        contract,
        expected_execution_b_git_commit=args.execution_b_git_commit,
    )
    original_evidence = validate_retained_v1_failure(contract.parent, args.data_root)
    inventory_evidence = validate_retained_inventory_repair_failure(
        contract,
        args.data_root,
    )
    new_roots = (
        validate_new_run_roots_absent(contract, args.data_root)
        if args.command == "run"
        else None
    )
    ...
    def cached_original(checked: Any, checked_root: Path) -> Mapping[str, Any]:
        if checked is not contract or Path(checked_root) != args.data_root:
            raise ValueError("cached original-evidence invocation drifted")
        return original_evidence

    def cached_inventory(checked: Any, checked_root: Path) -> Mapping[str, Any]:
        if checked is not contract or Path(checked_root) != args.data_root:
            raise ValueError("cached inventory-evidence invocation drifted")
        return inventory_evidence

    def cached_roots(checked: Any, checked_root: Path) -> Mapping[str, Any]:
        if (
            checked is not contract
            or Path(checked_root) != args.data_root
            or new_roots is None
        ):
            raise ValueError("cached new-root invocation drifted")
        return new_roots

    return execute_fresh16_claim_serialization_repair(
        mode=args.command,
        contract=contract,
        api=api,
        ...
        original_failure_validator=cached_original,
        inventory_failure_validator=cached_inventory,
        new_roots_validator=cached_roots if args.command == "run" else None,
    )
