#!/usr/bin/env python3
"""Run the frozen, CPU-only selected-image format census."""
        ...
        for batch in parquet.iter_batches(batch_size=8):
            for row in batch.to_pylist():
                if not isinstance(row, Mapping):
                    raise TypeError("GUIOdyssey parquet row must be one mapping")
                yield row_index, row
                row_index += 1
...
def _verify_checkout(root: Path, revision: str) -> None:
    ...
    if observed != revision:
        raise ValueError("formal census Git revision differs from checkout HEAD")
    status = subprocess.check_output(
        ["git", "-C", str(root), "status", "--porcelain"], text=True
    )
    if status:
        raise ValueError("formal selected-image census requires a clean Git checkout")
...
def _verify_runtime_versions(args: argparse.Namespace) -> None:
    ...
    if observed != expected:
        raise ValueError(f"selected-image census runtime versions drifted: {observed!r}")
...
def _assert_cpu_only_import_state() -> None:
    ...
    if loaded:
        raise RuntimeError(f"CPU-only census imported forbidden runtimes: {loaded!r}")
...
def _launch_workers(args: argparse.Namespace, staging: Path) -> None:
    ...
    if failures:
        raise RuntimeError(f"selected-image census workers failed: {failures!r}")


def _prepare_staging(output_root: Path) -> Path:
    if output_root.exists() or output_root.is_symlink():
        raise FileExistsError("selected-image census output root already exists")
...
def _validate_final_inventory(staging: Path) -> None:
    ...
    if {path.name for path in staging.iterdir()} != expected_root:
        raise ValueError("final census root inventory drifted")
    for directory, suffix in (("records", ".jsonl"), ("receipts", ".json")):
        root = staging / directory
        if not root.is_dir() or root.is_symlink():
            raise ValueError("final census directory inventory is invalid")
        expected = {f"worker-{index:02d}{suffix}" for index in range(4)}
        if {path.name for path in root.iterdir()} != expected:
            raise ValueError(f"final census {directory} inventory drifted")
        if any(not path.is_file() or path.is_symlink() for path in root.iterdir()):
            raise ValueError(f"final census {directory} contains a non-regular file")
    manifest = staging / "manifest.json"
    if not manifest.is_file() or manifest.is_symlink():
        raise ValueError("final census manifest must be one regular file")
    run_identity = staging / "run-identity.json"
    if not run_identity.is_file() or run_identity.is_symlink():
        raise ValueError("final census run identity must be one regular file")
...
def _orchestrate(args: argparse.Namespace, contract: Any) -> None:
    ...
    frozen_bindings = contract.data["bindings"]["frozen_inputs"]
    runtime_cli = {
    ...
    run_identity = {
        ...
        "source_audit": {
            name: binding["sha256"]
            for name, binding in sorted(frozen_bindings.items())
        },
        "versions": {
            "pillow": args.pillow_version,
            "pyarrow": args.pyarrow_version,
    ...
    if aggregate["observation_count"] != EXPECTED_OBSERVATION_COUNT:
        raise ValueError("final selected-image census denominator drifted")
    manifest = {
        "artifact": {
            "hf_mutation_count": 0,
            "intended_private_hf_repo": HF_REPO,
            "intended_tag": HF_TAG,
            "status": "PENDING_HF_UPLOAD",
        },
        "bindings": {
            "config_sha256": contract.config_sha256,
            "container_image_digest": args.container_image_digest,
            "freeze_b_v2_manifest_sha256": sha256_bytes(
                canonical_json_bytes(
                    contract.frozen_inputs["freeze_b_v2_manifest"], pretty=True
                )
            ),
            "git_revision": args.git_revision,
...
def main() -> None:
    ...
    _verify_executable(
        args.python_executable, expected_version=args.python_version
    )
