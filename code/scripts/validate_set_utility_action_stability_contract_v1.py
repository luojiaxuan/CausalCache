#!/usr/bin/env python3
"""Build or validate the train-only D1 action-stability source contract."""
...
def main() -> None:
    ...
    if args.emit_skeleton:
        payload = build_action_stability_source_v1_config_skeleton(
            repository_root=args.repository_root
        )
    else:
        contract = load_action_stability_source_v1_contract(
            repository_root=args.repository_root,
            config_path=args.config,
        )
        payload = validate_action_stability_source_v1_config(
            contract.data,
