...
def test_ungated_v4_w8_config_is_step_equivalent() -> None:
    """w8 变体与 ungated_kv_v4 唯一差别 = accumulation 4→2(配 world 8)。"""
    import json as json_module
    from pathlib import Path
    ...
    w8 = json_module.loads(
        (root / "causalcache_desktop_did_ungated_kv_v4w8.json").read_text(
            encoding="utf-8")
    )
    assert w8["training"]["gradient_accumulation_steps"] == 2
    w8_training = dict(w8["training"], gradient_accumulation_steps=4)
