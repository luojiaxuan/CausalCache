...
OUT=$BASE/twoarm/out-$ARMNAME
LOG=$BASE/twoarm/driver-$ARMNAME-$HOSTTAG.log
mkdir -p "$OUT" "$BASE/twoarm/logs"
  ...
  (
    ...
    for a in 1 2 3 4 5; do
      PYTHONPATH=$WORKERCODE:$OSWROOT /usr/bin/python3 \
        ...
        --cache-dir "$BASE/twoarm/cache-$ARMNAME" \
        >> "$BASE/twoarm/logs/$ARMNAME-shard$s.log" 2>&1
      code=$?
      last=$(grep -h '"event": "WORKER_COMPLETE"' "$BASE/twoarm/logs/$ARMNAME-shard$s.log" | tail -1)
