"""Byte-bound execution contract for the selected-image format census."""
...
STATUS = "CPU_ONLY_READ_ONLY_SELECTED_IMAGE_CENSUS_AUTHORIZED"
VALIDATION_STATUS = "VALID_SELECTED_IMAGE_FORMAT_CENSUS_V1_CONTRACT"
CANONICAL_CONFIG_PATH = (
    "code/configs/causalcache_set_utility_selected_image_format_census_v1.json"
)
RUNNER_PATH = "code/scripts/run_set_utility_selected_image_census.py"
...
HF_TAG = "phase1-b2-image-format-census-v1"
...
REQUIRED_REPOSITORY_SOURCE_PATHS = (
    ...
    "code/causalcache/set_utility_selected_image_census.py",
    "code/causalcache/set_utility_selected_image_census_contract.py",
...
def _expected_runtime() -> dict[str, Any]:
    return {
        ...
        "parquet_reader": "pyarrow_whole_selected_shards_images_column_on_selected_rows",
        "selected_row_access": "images_column_only_no_message_metadata_action_or_outcome_decode",
...
def _expected_output() -> dict[str, Any]:
    return {
        ...
        "worker_artifact": "canonical_jsonl_plus_receipt",
    }
...
def _validate_inputs(documents: Mapping[str, Mapping[str, Any]], hashes: Mapping[str, str]) -> None:
    ...
    if (
        ...
        or _mapping(freeze.get("repair"), label="Freeze-B v2 repair").get("worker_count")
        != WORKER_COUNT
    ):
...
class SelectedImageCensusContract:
...
def validate_execution_config(
    ...
    _exact_keys(
        ...
        label="selected-image census config",
    )
    if (
        ...
        raise ValueError("selected-image census config identity drifted")
    ...
    if dict(_mapping(config.get("authorization"), label="authorization")) != _expected_authorization():
    ...
    _validate_inputs(documents, hashes)
    sources = _sequence(bindings.get("runtime_import_closure"), label="runtime import closure")
    ...
    for index, path in enumerate(REQUIRED_REPOSITORY_SOURCE_PATHS):
        _validate_binding(root, sources[index], label=f"runtime source {index}", expected_path=path)
    return SelectedImageCensusContract(
        data=config,
...
def load_execution_contract(
    *, repository_root: str | Path, execution_config_path: str | Path
    ...
    payload = _safe_repository_file(root, CANONICAL_CONFIG_PATH, label="execution config").read_bytes()
    config = _strict_json(payload, label="selected-image census config")
...
def validate_runtime_cli_values(
    contract: SelectedImageCensusContract, values: Mapping[str, Any]
) -> None:
    expected_keys = {
    ...
    if not isinstance(values["git_revision"], str) or _GIT_REVISION.fullmatch(values["git_revision"]) is None:
...
def validate_cpu_only_source(*, repository_root: str | Path) -> dict[str, Any]:
    ...
    for relative in (RUNNER_PATH, "code/causalcache/set_utility_selected_image_census.py"):
        source = _safe_repository_file(root, relative, label=relative).read_text(encoding="utf-8")
        tree = ast.parse(source, filename=relative)
        for node in ast.walk(tree):
            if isinstance(node, ast.Import):
                roots = {alias.name.split(".")[0] for alias in node.names}
                if roots & forbidden_modules:
                    raise ValueError(f"CPU-only source imports forbidden module in {relative}")
                ...
                if name in forbidden_names:
                    raise ValueError(f"CPU-only source calls forbidden interface in {relative}")
            elif (
    ...
    return {
        ...
        "ocr_count": 0,
        "protocol_id": PROTOCOL_ID,
        "status": "VALID_CPU_ONLY_SELECTED_IMAGE_CENSUS_SOURCE",
    }


def validation_summary(contract: SelectedImageCensusContract) -> dict[str, Any]:
    return {
