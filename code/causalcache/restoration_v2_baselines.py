...
def _mean_pool_and_l2_normalize(
    last_visual_output: Iterable[Iterable[Real]],
    name: str,
) -> tuple[float, ...]:
    if isinstance(last_visual_output, (str, bytes, bytearray, Mapping)):
        raise TypeError(f"{name} must be a two-dimensional numeric iterable")
    try:
        rows = iter(last_visual_output)
        first_raw = next(rows)
    except StopIteration as error:
        raise ValueError(f"{name} must contain at least one visual token") from error
    except TypeError as error:
        raise TypeError(f"{name} must be a two-dimensional numeric iterable") from error

    def numeric_row(raw_row: Any, row_index: int) -> tuple[float, ...]:
        if isinstance(raw_row, (str, bytes, bytearray, Mapping)):
            raise TypeError(f"{name}[{row_index}] must be a numeric iterable")
        try:
            raw_values = tuple(raw_row)
        except TypeError as error:
            raise TypeError(f"{name}[{row_index}] must be a numeric iterable") from error
        if not raw_values:
            raise ValueError(f"{name} hidden dimension must be positive")
        return tuple(
            _finite_float(value, f"{name}[{row_index}][{column_index}]")
            for column_index, value in enumerate(raw_values)
        )

    first = numeric_row(first_raw, 0)
    sums = list(first)
    row_count = 1
    for row_index, raw_row in enumerate(rows, start=1):
        row = numeric_row(raw_row, row_index)
        if len(row) != len(sums):
            raise ValueError(f"{name} rows must share one hidden dimension")
        for column_index, value in enumerate(row):
            sums[column_index] += value
        row_count += 1
    mean = tuple(value / row_count for value in sums)
    norm = math.sqrt(math.fsum(value * value for value in mean))
    if norm == 0.0:
        raise ValueError(f"{name} mean-pooled vector must have positive L2 norm")
    return tuple(value / norm for value in mean)


def _cosine_of_normalized_vectors(
    left: tuple[float, ...],
    right: tuple[float, ...],
) -> float:
    if len(left) != len(right):
        raise ValueError("policy vision embeddings must share one hidden dimension")
    value = math.fsum(a * b for a, b in zip(left, right, strict=True))
    return min(1.0, max(-1.0, value))


def frozen_policy_vision_embedding_similarity_baseline(
    *,
    event_last_visual_outputs: Mapping[int, Iterable[Iterable[Real]]],
    current_last_visual_output: Iterable[Iterable[Real]],
) -> BaselineSelection:
    outputs_by_event = _candidate_mapping(
        event_last_visual_outputs,
        "event_last_visual_outputs",
    )
    current = _mean_pool_and_l2_normalize(
        current_last_visual_output,
        "current_last_visual_output",
    )
    scores = {
        step_id: _cosine_of_normalized_vectors(
            _mean_pool_and_l2_normalize(
                outputs_by_event[step_id],
                f"event_last_visual_outputs[{step_id}]",
            ),
            current,
        )
        for step_id in CANDIDATE_EVENT_STEP_IDS
