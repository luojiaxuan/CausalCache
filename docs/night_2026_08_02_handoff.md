...
2. **部署链路有一个可修的校准缺陷**:两塔中的 readout 塔在部署侧被 mask(它需要挂探针
   ...
   但让**一切阈值/早停判据失效**。
3. **修复改变了行为,但没有转化为同域闭环收益**。两轮同批双臂:
   第 1 轮 **+2.22pp**、第 2 轮 **−0.55pp**,**两轮平均 +0.83pp,CI [−1.25,+3.05],p=0.24**。
   第 1 轮那个正向**没有复现**。行为改变是确凿的(弃权从 97.4% 全弃权变为每步提升
   1-2 张、25.7% 真正偏离 recent),但成功率上测不出来。
...
同批双臂两轮均已完成并归约,见 `data/results/indomain_twoarm_v1/README.md`
文末「两轮结果」。

**同一 Recent-4 臂在三个时间窗分别是 32.96% / 30.47% / 29.92%(跨轮差最大 3.0pp)**
——这是本次最该记住的数字:**任何跨轮比较都不可信**,包括论文里现有的同域对比。
...
- 同域两轮平均 **+0.83pp(p=0.24)**,论文中**不可表述为"已修复"或"小幅正向"**。
  诚实表述:两轮 n=722 仍不足以判定,真实效应若存在也小于约 3pp。
- 我在第 1 轮曾把 writer 域 0:4→3:2 报为"目标故障修复"。总体效应未复现后,
  该单域信号(n=23)同样不可作为证据——**这是"看到想看的模式"的陷阱,
  我当时应更保守**。
