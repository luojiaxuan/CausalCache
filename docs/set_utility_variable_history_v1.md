...
- 首次 16-worker launch 暴露 official-tools encoder 仍硬编码 historical max batch=2；16 个 empty-coalition
...
- 修复后 8xH200 正式 run 使用 24-way workers、每卡 3 个 in-flight process；初始检查已完成 86 states / 3,232
  labels，覆盖 `n_t=5..9`，全部 terminal completed、KL 有限且非负。每个 state、reference 与 coalition
  microbatch 都原子落盘；worker/GPU 重新分配不会丢失已有进度。
