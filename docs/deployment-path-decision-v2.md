...
### OSWorld-Verified(361 任务,desktop=训练平台,**全满员终值**)

| 臂 | 成功率 | 配对 |
|---|---|---|
| 冻结 B0 | 19.94%(72/361) | — |
| HGKV recent-B4 | 32.96%(119/361) | vs B0 **+13.02** [8.59,17.73] ✓ |
| **HGKV+selector 单遍** | **34.07%**(123/361) | vs B0 **+14.13** [9.42,18.84] ✓;vs recent +1.11 [−2.49,+4.71] n.s. |

**跨平台互补结构**(paper 核心洞察):OSWorld 任务相关状态多在近期窗口 →
recent 吃掉大部分记忆收益、选择中性偏正;MobileWorld 跨应用任务相关帧在
远处 → recent 无效(+0.85 n.s.)、只有 selected 恢复拿到收益(两遍 +6.41 ✓)。
mobile 名册分层佐证:单遍失效集中在单应用对照组(−3.6pp,误换帧),
memory_candidate 组两口径皆正(两遍 +7.3 / 单遍 +4.8)。
