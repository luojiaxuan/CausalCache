...
## v5a(selector-allocation 语料,last_8)门控结果(2026-07-29)

双口径,6 checkpoint 全曲线(A_r 漂移全程 |≤0.004| 远在帽内):

| ckpt | v4-b1 经典(94 组) | v5-hit 多槽 heldout(130 组) |
|---|---|---|
| 50 | +0.0026 | +0.0012 |
| 100 | +0.0118 | +0.0099 |
| 150 | +0.0186 | +0.0164 |
| 200 | +0.0165 | +0.0142 |
| 250 | +0.0178 | +0.0165 |
| **300** | **+0.0218 [.016,.029]** | **+0.0193 [.013,.026]** |

- 结论 1:换成部署分布(多槽 S*)训练**没有损失经典口径选择性**
  (step300 +0.0218 ≈ v4 主线 +0.0224)。
- 结论 2:多槽部署形态上的选择性首次可测且为正(+0.0193)——v4 只在
  k=1 上测过。收益拐点在 step150(50→150 贡献 ~0.016,150→300 仅
  +0.003),300 步仍在缓升未饱和;按冻结规则选 step300。
- 下一步(用户指定流程):用 v5a-step300 当教师重打 selector 标签
  (singleton+sets,同 60% 状态子集保持三教师可比)→ 重训 selector →
  闭环。v5a-full(last_36)训练并行中(step 80)。

## v5a-full(last_36 全深度)门控(2026-07-29):位置假设成立
