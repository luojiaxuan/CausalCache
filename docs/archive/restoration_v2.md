...
[`restoration_v2_constructor_preflight`](../../data/results/archive/restoration_v2_constructor_preflight/README.md)。
device-side executor dispatch 也已对 14/14 cases 正式通过，negative actuation control 为 HTTP 500，见
[`restoration_v2_executor_dispatch`](../../data/results/archive/restoration_v2_executor_dispatch/README.md)。action round
...
[`restoration_v2_derived_artifact`](../../data/results/archive/restoration_v2_derived_artifact/)。
...
实现已冻结于 `code/causalcache/restoration_v2_baselines.py` 与
`code/causalcache/policy/gui_owl_v2_vision.py`。random 严格枚举 lexicographic 六个 2-of-4 subsets 并计算
解析均值，不借用 attribution seed；OCR+RGB 直接消费 derived artifact 的 uncapped full-screen tokens 与 pinned
256x256 RGB bytes，不能由 capped summary delta 反推。policy-vision 只允许
`Qwen3VLForConditionalGeneration.get_image_features(...).pooler_output` 的 final main merger 输出，排除
pre-merger `last_hidden_state` 和 layer 8/16/24 DeepStack outputs；BF16 feature 在 accelerator 上转 FP32 后
mean/L2/cosine。完整 snapshot 与 Transformers source verifier、11-file source manifest 已通过，dependency 6
已闭合。
...
[`../../data/results/archive/restoration_v2_gpu_compute_audit/`](../../data/results/archive/restoration_v2_gpu_compute_audit/)。
...
[`restoration_v2_substrate_screening`](../../data/results/archive/restoration_v2_substrate_screening/)。任何输出接口救援必须
