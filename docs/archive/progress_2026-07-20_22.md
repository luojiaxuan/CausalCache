# 项目进展(存档:2026-07-22 及更早)

> 本文件是 [`docs/progress.md`](../progress.md) 在 history-gated mainline 成为主线(2026-07-23)之前的全部旧条目,逐字迁移,不再更新。涵盖 2026-07-14 至 2026-07-22 的 policy 选型、restoration v2、gate v1、independent confirm、set-utility/selector、margin-SFT 与 OSWorld 基建各时代;时代级摘要见 [`docs/archive/README.md`](README.md)。
...
- 轻量记录见 [`data/results/archive/set_utility_capacity_and_prefetch_v1/`](../../data/results/archive/set_utility_capacity_and_prefetch_v1/README.md)。
...
- 实现与冻结参数见 [`docs/archive/set_utility_decision_distillation_v2.md`](set_utility_decision_distillation_v2.md)。
...
- learned-minus-recent 95% CI 分别为 `[-0.02673,0.00954]` 与 `[-0.03491,0.01237]`；verdict=
  ...
  matched-NLL；
- 轻量结果见
  [`data/results/archive/set_utility_contextual_tune_on_policy_enriched_v1/`](../../data/results/archive/set_utility_contextual_tune_on_policy_enriched_v1/README.md)；
...
- 下一步优先 contextualized GUI-Owl hidden states、保留多 latent tokens 进入 set interaction，并补
  ...
  [`data/results/archive/set_utility_b4_oracle_diagnostic_v2/`](../../data/results/archive/set_utility_b4_oracle_diagnostic_v2/README.md)。
...
- full result 位于 Hyper00
  ...
  [`data/results/archive/set_utility_heldout_v1/`](../../data/results/archive/set_utility_heldout_v1/README.md)，完整 artifact 已发布到
...
- Git-safe finalization 见
  [`../../data/results/archive/set_utility_processor_freeze_v2_image_contract_repair_publication_v1/`](../../data/results/archive/set_utility_processor_freeze_v2_image_contract_repair_publication_v1/)。原 pending card/summary、外置 receipt 与两份 replay
...
- 该分解不是新 GO，不能改变父 `NO_GO_INDEPENDENT_CONFIRM`，也不能解锁 closed-loop、matched-NLL、sealed
  ...
  [`../../data/results/archive/independent_confirm20_failure_decomposition_v1/`](../../data/results/archive/independent_confirm20_failure_decomposition_v1/)，完整边界见
...
- 406-file deterministic USTAR SHA256 为
  ...
  reducer 再次返回 PASS；
- 轻量 Git 结果位于 `data/results/archive/restoration_v2_2_label_expansion_substrate_v1/`。本步只解锁 expansion
...
- 原 v1 不得 retry/resume、调阈值或追认为 formal PASS；canonical PASS archive/HF repo/tag 均未创建。轻量
  failure binding 位于 `data/results/archive/restoration_v2_2_expansion_exact_labels_v1_attempt/`；
...
- 当前 archive 只在 Hyper00 staging，尚不是 reusable SoT。下一步冻结 P1 private-HF publication，完成
  ...
  `data/results/archive/restoration_v2_2_expansion_exact_labels_invalid_forensic_v1/`。
...
- 结果见 `data/results/archive/qwen_policy_smoke/`。这只验证 policy forward 链路，不能说明 restoration 有收益。
...
- 结果见 `data/results/archive/qwen_policy_coverage/`。这不是对 CausalCache 机制的 falsification，而是 backbone selection 的负结果。
...
- 结果见 `data/results/archive/ui_tars_policy_coverage/`。这仍是 backbone selection 的负结果，不是 CausalCache 方法效果。
...
- canonical smoke 的 summary-only logits shape 为 `[1, 526, 152064]` 且全部 finite；只恢复 event 2 时唯一 action 与 recorded `type_text("Cryptocurrency Market")` 匹配；结果见 `data/results/archive/open_cua_policy_smoke/`，不作为 attribution 效果证据。
...
- 拒绝该 candidate 进入 attribution pilot，结果见 `data/results/archive/open_cua_policy_coverage/`。
...
- 拒绝该 candidate 进入 attribution pilot，结果见 `data/results/archive/showui_policy_coverage/`；
...
- 结果见 `data/results/archive/gui_owl_native_smoke/`。下一步启动 AndroidWorld emulator/reward smoke。
...
- 结果见 `data/results/archive/androidworld_environment_smoke/`。下一步先提交冻结 task partition manifest，
...
- 单实例耗时 78.675 秒，最多 4 张可见截图、2,040 input tokens、16.90 GiB peak allocated GPU
  memory；完整 trace 见 `data/results/archive/gui_owl_androidworld_validation_smoke/`；
...
- rollout 被立即中止，16 个 checkpoint 标记为 implementation-invalid 并从正式 gate 排除；诊断摘要见
  `data/results/archive/gui_owl_androidworld_validation_attempt1/`；
...
- 第二次运行整体标记为 configuration-invalid，不用于拒绝 GUI-Owl，也不改变 validation plan、
  ...
  `data/results/archive/gui_owl_androidworld_validation_attempt2/`；
...
- 该 fixture 的横向截图 token 数高于 AndroidWorld；1080×2400 AndroidWorld processor replay
  仍固定为 `[1,150,68]` / 2,550 tokens。结果见 `data/results/archive/gui_owl_native_resolution_smoke/`。
...
- GUI-Owl 正式拒绝为 validated teacher，不升级 AndroidWorld attribution contract，不在该 stack
  上生成 restoration labels 或训练 gate；结果见 `data/results/archive/gui_owl_androidworld_validation/`；
...
- 新增 `AGENTS.md`、`code/README.md`、`data/README.md` 与 `docs/archive/execution.md`，固定每步
...
- Hyper01 单 H200 在 Git `e40a0c780c96dda1d43ac5ae1469ccca86d9887d` 运行，model/data revision、
  ...
  `data/results/archive/gui_owl_1_5_8b_think_smoke_strict/run_manifest.json`；
...
- Hyper01 单 H200 在 Git `72c59c4d5f2f9c4f7eb42758d6606b1736e6121d` 以原 model/data/fixture/
  ...
  `data/results/archive/gui_owl_1_5_8b_think_smoke/run_manifest.json`；
...
- 本次生成 0 个 restoration distances，`valid_for_diagnostic=false`，不解释为方法结果；轻量失败摘要见
  `data/results/archive/go_no_go_diagnostic_v1_attempt1/`；
...
- canonical compact result 与全部 coalition distances 已写入
  `data/results/archive/go_no_go_diagnostic_v1/`。这两个 states 来自已观察的 matched subset，明确禁止升级为 paper
...
- Hyper00 随后用 exact UI-TARS snapshot 在旧 9-decision artifact 运行 behavioral anchor；9/9 parsed、4/9
  ...
  Aries。compact result 位于 `data/results/archive/ui_tars_hyper00_hardware_anchor/`；
...
- raw per-decision summary 与 monitor 上传 private HF
  ...
  SHA256，轻量结论见 `data/results/archive/independent_reference_gate_v1/`；
...
- scientific config SHA256 为
  ...
  完整说明见 `docs/archive/restoration_v2.md`。这一步没有运行 GPU 或产生新 artifact。
...
- 真实 pinned AndroidWorld `new_json_action.JSONAction` constructor 和 device-side executor dispatch 尚未
  ...
  artifact。完整语义见
  `docs/archive/restoration_v2_interfaces.md`。
...
- 结果绑定 interface manifest SHA256 `02744d82...`、runtime image repo digest `6a8f60af...`、完整 argv、
  host/container 与起止时间，见 `data/results/archive/restoration_v2_constructor_preflight/`；
...
- canonical offline verdict 为 `PASSED_EXECUTOR_DISPATCH`，summary SHA256
  ...
  `data/results/archive/restoration_v2_executor_dispatch/`；
...
- 完成态 `data/manifests/restoration_v2_ocr_backend.json` 绑定 14 个 Git source、HF model/dataset 两个
  ...
  `data/results/archive/restoration_v2_ocr_backend/real_screen_summary.json`。离线 outcome 仍为
...
- Hyper00 在 UTC `15:20:56.587--15:31:30.534Z` 对该 immutable projection 完成第三次 210-record OCR
  ...
  `data/results/archive/restoration_v2_derived_artifact/`。
...
- restoration v2 executor evidence：Git `data/results/archive/restoration_v2_executor_dispatch/`，formal verdict
...
- selection-biased go/no-go compact result：Git `data/results/archive/go_no_go_diagnostic_v1/`；raw 209 KiB debug
...
- replay 没有 raw-text Git exposure、model import/load、forward/generation、retry、top-up 或 confirm access。
  ...
  `data/results/archive/restoration_v2_parser_compatibility/`；结果回归加入 committed-result/golden binding 后，全量
...
- 7,609,803-byte raw JSON（SHA256 `5349ffc6b91bf93ed26d25104fe6c907f31ccc497007a5c0ae6ed5ddf5c84991`）
  ...
  均已复核；
- Git 只保存 `data/results/archive/restoration_v2_1_processor_preflight/` 的轻量 manifest/说明。该 PASS 只关闭
...
- Git 只保存 `data/results/archive/restoration_v2_1_interface_pilot/` 的 34-file inventory/binding 与说明。manifest
...
- canonical 94-file deterministic USTAR 为 962,560 bytes，SHA256
  ...
  位于 `data/results/archive/restoration_v2_1_full_45_substrate/`；
...
- 旧 cached real table 的四个 cases 仍全部 greedy=exact；step 8 / 1024 的 2 positive + 19 negative pair
  ...
  offline diagnostic；
- 轻量 summary/report 位于 `data/results/archive/subset_search_ablation_v1/`。下一步 commit/push result，再从 clean
...
- 完整说明见 `docs/archive/spatial_reference_audit_v1_validation_repair.md`。repair source commit/push、远端 clean-main
...
- Git 只回写 `data/results/archive/spatial_reference_audit_v1/` 的 compact summary/artifact binding。下一步不是
...
- machine-readable contract 为 `code/configs/causalcache_restoration_v2_2_eager.json`，完整说明为
  `docs/archive/restoration_v2_2_eager.md`。formal freeze 已从 clean pushed
...
- Git compact result 位于 `data/results/archive/restoration_v2_2_eager_full_45_substrate/`。eager 已通过，因此不进入
...
- v1 root/ledger 永久保留且 retry/resume=false。Git compact failure binding 位于
  `data/results/archive/restoration_v2_2_eager_labels_v1_attempt/`，没有可上传的 label dataset，HF repo/tag 未创建；
...
- Git compact result 位于 `data/results/archive/restoration_v2_2_eager_labels_v2/`。当前只闭合 offline oracle/labels；
...
- canonical result 位于 `data/results/archive/restoration_v2_2_selector_geometry_v2_repair/`，status 为
...
- GPU、OCR inference/model load、policy/policy-vision forward、teacher/KL、gate、matched-NLL 与 closed-loop
  ...
  `docs/archive/restoration_v2_2_ocr_rgb_baseline.md`。
...
- canonical output 与 staging directory 均不存在；v1 identity 不得重跑。compact evidence 见
  `data/results/archive/restoration_v2_2_ocr_rgb_baseline_v1_attempt/`；
...
- formal v2 output 仍未生成。下一步从包含本 source freeze 的 clean pushed `main` 在 Hyper00 CPU runtime 执行，
  ...
  `docs/archive/restoration_v2_2_ocr_rgb_baseline_v2_identity_repair.md`。
...
- confirm/test、gate、matched-NLL 与 closed-loop 仍为 0。source-only validation 和正式执行边界见
  `docs/archive/restoration_v2_2_policy_vision_baseline.md`；source push 后才允许 formal GPU run。正式入口为
...
- v1 不重试。轻量 evidence 位于
  `data/results/archive/restoration_v2_2_policy_vision_baseline_v1_attempt/`；下一步先冻结只接受 pinned
...
- v2 ledger SHA256 为 `492e7c7a...4502`，停止的 container `sglang-omni-jaxan-07170602` 与 ledger 均保留。
  ...
  `data/results/archive/restoration_v2_2_policy_vision_baseline_v2_gpu_uuid_repair_attempt/`。
...
- bounded CPU diagnostic 只做上述四键 projection，其余 rows、summary、labels、witness 不变，三份 artifact
  ...
  `data/results/archive/restoration_v2_2_policy_vision_baseline_v3_cpu_validation_attempt/`。
...
结果见 `data/results/archive/gate_v1_fresh16_claim_serialization_repair_v1/`。只读 failure decomposition 也已完成、发布并
...
- repair runtime 的 GPU/model/policy/teacher/KL/gate/matched-NLL/closed-loop/confirm/test/remote-mutation operation
  count 全部为 0。轻量结果位于
  `data/results/archive/restoration_v2_2_expansion_exact_labels_scientific_repair_v1/`；local artifact 还不是 reusable SoT，
...
- 该里程碑令 `formal_label_loader_eligible=true`、`gate_training_unlocked=true`，但只清除 formal-58
  ...
  `data/results/archive/restoration_v2_2_expansion_exact_labels_scientific_repair_publication_v1/`。
...
- 旧 claim 不删除、不覆盖、不续跑。下一步必须用独立 transport-repair overlay 绑定旧 claim 与 Git-pinned
  ...
  `data/results/archive/gate_v1_formal58_cache_v1_attempt/`。
...
- 同一 committed B 的只读 `validate` 返回
  ...
  `data/results/archive/gate_v1_formal58_cache_transport_repair_v1/`；下一步仍需要单独的 train-only gate source/execution
...
- 正式 `run` 返回 `VALID_GATE_V1_FORMAL58_TRAIN_PUBLICATION_V1`，完成态只读 replay 返回
  ...
  `data/results/archive/gate_v1_formal58_train_v1/`；
...
- 当前尚未读取任何真实 fresh-16 trajectory/OCR/image/label，也没有 GO verdict。下一步是 commit/push
  ...
  `docs/archive/gate_v1_fresh16_evaluation.md`。
...
- 旧 v1 不删除、不覆盖、不续跑，也不解释为 selector NO-GO。轻量证据在
  `data/results/archive/gate_v1_fresh16_evaluation_v1_attempt/`。下一步另立 full-remote-inventory repair A/B，绑定旧 failure
...
- 本 milestone commit/push 即 repair A；随后从 clean pushed A 重放 source validator，机械生成唯一
  ...
  `docs/archive/gate_v1_fresh16_inventory_repair.md`。
...
- 轻量证据见 `data/results/archive/gate_v1_fresh16_inventory_repair_v1_attempt/`。旧 state/artifact/runtime-receipt
...
- 该 attempt 永久 `INVALID_INDEPENDENT_CONFIRM20_V1_CUDA_FORK_BOUNDARY`，不是 selector NO-GO；同一 identity
  不重试，远端 payload 不覆盖。轻量证据见 `data/results/archive/independent_confirm20_v1_attempt/`；
...
- Git-safe failure evidence 位于
  `data/results/archive/set_utility_processor_freeze_execution_cf_v1_attempt/`。下一步先做全部 18,792 selected
...
- Git-safe summary 位于 `data/results/archive/set_utility_token_predictor_v2_partial/`。dataset artifacts 已发布到
...
- 预注册判定:`oracle_greedy_minus_recent_macro` 的 95% CI `lower>0.05` → headroom confirmed;
  ...
  [`docs/archive/set_utility_long_history_oracle_v1.md`](set_utility_long_history_oracle_v1.md)。
...
- artifact:reducer summary 入库 `data/results/archive/set_utility_long_oracle_v1/`;完整 payload 已存
...
- 审计脚本与完整数字入库:`code/scripts/audit_guiodyssey_official_splits.py`、
  `data/results/archive/guiodyssey_official_split_audit_v1/`;paper 辩护口径(不做官方 benchmark、我们的
...
- full result content SHA=`043d3e12...d4c86`、file SHA=`4dd81748...284c9`；轻量结果见
  `data/results/archive/set_utility_decision_distillation_v2_epoch1_tune/`。full selections/schedule/labels/result 与
...
- 结果只作 in-sample 机理诊断,不进 gate/评测/paper 主表。
  [结果](../../data/results/archive/set_utility_long_oracle_insample_v1/README.md);selector 补丁 `b98e0ec`。
...
- 实现与 13 个 focused tests 已在 Hyper00 PyTorch 2.13 container 通过；合同见
  [`docs/archive/set_utility_tune_long_oracle_v1.md`](set_utility_tune_long_oracle_v1.md)。
...
- 完整 reducer 见 [`data/results/archive/set_utility_tune_long_oracle_v1/`](../../data/results/archive/set_utility_tune_long_oracle_v1/README.md)；
...
- 完整设计与止损边界见 [`docs/archive/set_utility_direct_marginal_v3.md`](set_utility_direct_marginal_v3.md)。
...
- 结果：[`data/results/archive/set_utility_direct_marginal_v3_fit_probe_v1/`](../../data/results/archive/set_utility_direct_marginal_v3_fit_probe_v1/README.md)。
...
- Hyper00/Hyper01 的资源授权更新为每台最多 8 GPU，供后续经新研究决策授权的并行任务使用；当前不以资源
  扩张绕过科学 gate。
- 结果：
  [`data/results/archive/set_utility_direct_marginal_v3_fit_probe_v2_rank_repair/`](../../data/results/archive/set_utility_direct_marginal_v3_fit_probe_v2_rank_repair/README.md)。
...
- 合同：[`docs/archive/set_utility_direct_marginal_v3_stage_b_v1.md`](set_utility_direct_marginal_v3_stage_b_v1.md)。
...
- 结果：[`data/results/archive/set_utility_direct_marginal_v3_stage_b_v1/`](../../data/results/archive/set_utility_direct_marginal_v3_stage_b_v1/README.md)。
...
- 结果：[`data/results/archive/set_utility_direct_marginal_v3_fixed_tune_v1/`](../../data/results/archive/set_utility_direct_marginal_v3_fixed_tune_v1/README.md)。
...
- 建议 Stage-A' 决策对齐合取门槛(top-1>=0.80 / top-4>=0.95 / B1>=0.90×oracle / STOP=1.0;v1 冻结
  ...
  [`docs/archive/set_utility_direct_marginal_v3_stage_a_gate_review.md`](set_utility_direct_marginal_v3_stage_a_gate_review.md);
...
- 用既有 tune truth 零成本模拟确定性 deferral 部署(冻结 checkpoint,不重训):**H1(B2 与
  ...
  [模拟记录](../../data/results/archive/set_utility_hybrid_deferral_sim_v1/README.md)。
...
- 记录见 [`data/results/archive/exploratory_closed_loop_validation12_local_arms_v1/`](../../data/results/archive/exploratory_closed_loop_validation12_local_arms_v1/README.md);
